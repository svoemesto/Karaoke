 ...
 * - [Comparable] — сортировка по [com.svoemesto.karaokeapp.model.SongField].
 *
...
//    }
    ...
    var formattedTextSong: String
        get() {
            val txt = fields[SongField.FORMATTED_TEXT_SONG] ?: ""
            return txt
        }
        set(value) {
            fields[SongField.FORMATTED_TEXT_SONG] = value
        }

    var formattedTextTabs: String
        get() {
            val txt = fields[SongField.FORMATTED_TEXT_TABS] ?: ""
            return txt
        }
        set(value) {
            fields[SongField.FORMATTED_TEXT_TABS] = value
        }

    var formattedTextChords: String
        get() {
            val txt = fields[SongField.FORMATTED_TEXT_CHORDS] ?: ""
            return txt
        }
        set(value) {
                ...
                PROJECT_ROOT_FOLDERS.forEach { rsf ->
                    ...
                    folders.forEach { folder ->
                        ...
                        if (File(candidate).exists()) {
                            ...
                            val sql = "UPDATE tbl_songs SET root_folder = ? WHERE id = ?"
    ...
    var ms: Long
        get() {
            val value = fields[SongField.MS]?.toLongOrNull() ?: 0L
            return if (value == 0L) {
                ...
                val durationFromAudioFileMs =
                    try {
                        ...
                        fields[SongField.MS] = tmp.toString()
                        val sql = "UPDATE tbl_songs SET song_ms = ? WHERE id = ?"
                        val connection = database.getConnection()
                        if (connection == null) {
        ...
        set(value) {
            fields[SongField.MS] = value.toString()
        }
...
//    }
    ...
    var tags: String = ""
    var fields: MutableMap<SongField, String> = mutableMapOf()
...
//        }
    ...
    override var id: Long
        get() = fields[SongField.ID]?.toLongOrNull() ?: 0L
        set(value) {
            fields[SongField.ID] = value.toString()
        }
    val idStatus: Long get() = fields[SongField.ID_STATUS]?.toLongOrNull() ?: 0L
    ...
    var sourceText: String
        get() {
            val txt = fields[SongField.SOURCE_TEXT] ?: ""
            return txt
        }
        set(value) {
            fields[SongField.SOURCE_TEXT] = value
        }
    ...
    var resultText: String
        get() {
            val txt = fields[SongField.RESULT_TEXT] ?: ""
            return txt
        }
        set(value) {
            fields[SongField.RESULT_TEXT] = value
        }
...
//                fields[SongField.SOURCE_TEXT] = Json.encodeToString(data)
//                saveToDb()
                data
            }
        }
    ...
    var sourceMarkers: String
        get() =
            if (fields[SongField.SOURCE_MARKERS] == null ||
                fields[SongField.SOURCE_MARKERS] == ""
            ) {
                "[[]]"
            } else {
                fields[SongField.SOURCE_MARKERS]!!
        ...
        set(value) {
            fields[SongField.SOURCE_MARKERS] = value
        }
...
//    val pathToFolderMP3Karaoke: String  get() = "$PATH_TO_STORE_FOLDER/MP3_Karaoke/${author} MP3".rightFileName()
    ...
    val time: String get() = fields[SongField.TIME] ?: ""
...
//        if (fields.contains(SongField.FORMAT)) {
//            "${fileName}.${fields[SongField.FORMAT]}"
//        } else {
//            if (fields.contains(SongField.AUDIOSONG)) {
//                fields[SongField.AUDIOSONG] ?: ""
//            } else {
//                "${fileName}.flac"
//            }
//        }
...
//                pict.preview = preview
        ...
        if (pic != null && !pictureAuthorReady) {
...
//                pict.preview = preview
    ...
    val audioMusicFileName: String get() =
        if (fields.contains(SongField.AUDIOMUSIC)) {
            fields[SongField.AUDIOMUSIC] ?: ""
        } else {
    ...
    val audioVocalFileName: String get() =
        if (fields.contains(SongField.AUDIOVOCALS)) {
            fields[SongField.AUDIOVOCALS] ?: ""
        } else {
    ...
    val audioBassFileName: String get() =
        if (fields.contains(SongField.AUDIOBASS)) {
            fields[SongField.AUDIOBASS] ?: ""
        } else {
    ...
    val audioDrumsFileName: String get() =
        if (fields.contains(SongField.AUDIODRUMS)) {
            fields[SongField.AUDIODRUMS] ?: ""
        } else {
    ...
    var rootId: Long
        get() {
            val txt = fields[SongField.ROOT_ID] ?: "0"
            return txt.toLong()
        }
        set(value) {
            fields[SongField.ROOT_ID] = value.toString()
        }

    var audioParentId: Long
        get() = (fields[SongField.AUDIO_PARENT_ID] ?: "0").toLongOrNull() ?: 0L
        set(value) {
            fields[SongField.AUDIO_PARENT_ID] = value.toString()
        }

    /**
     * Ссылка на альбом (`tbl_albums.id`) — `null`, если песня не привязана к альбому (сингл).
     * Настоящий FK (`ON DELETE SET NULL`), в отличие от свободнотекстовых [author]/[album]/[year].
     *
     * @see specs/011-album-song-rename/data-model.md
     */
    var albumId: Long?
        get() = fields[SongField.ALBUM_ID]?.toLongOrNull()
        set(value) {
            fields[SongField.ALBUM_ID] = value?.toString() ?: ""
        }

    var audioSimilarityPercent: Int
        get() = (fields[SongField.AUDIO_SIMILARITY_PERCENT] ?: "0").toIntOrNull() ?: 0
        set(value) {
            fields[SongField.AUDIO_SIMILARITY_PERCENT] = value.toString()
        }

    var audioDeltaMs: Long
        get() = (fields[SongField.AUDIO_DELTA_MS] ?: "0").toLongOrNull() ?: 0L
        set(value) {
            fields[SongField.AUDIO_DELTA_MS] = value.toString()
        }

    @get:JsonIgnore
    var audioCompareHistory: String
        get() = fields[SongField.AUDIO_COMPARE_HISTORY]?.ifBlank { "[]" } ?: "[]"
        set(value) {
            fields[SongField.AUDIO_COMPARE_HISTORY] = value
        }
    ...
    var free: Boolean
        get() {
            val txt = fields[SongField.FREE] ?: false.toString()
            return txt == true.toString()
        }
        set(value) {
    ...
    var playerReadinessFlags: String
        get() = fields[SongField.PLAYER_READINESS_FLAGS]?.ifBlank { "{}" } ?: "{}"
        set(value) {
            fields[SongField.PLAYER_READINESS_FLAGS] = value
        }
    ...
    // isPubliclyWatchable ниже (specs/083-auto-news-song-release), чтобы не заводить третью
    ...
    // «песня стала публично доступна всем», см. specs/083-auto-news-song-release/spec.md FR-009).
    ...
    val versionBoostyFiles: Int get() = (fields[SongField.VERSION_BOOSTY_FILES]?.nullIfEmpty() ?: "0").toInt()
    val idVk: String get() = fields[SongField.ID_VK]?.nullIfEmpty() ?: ""
    val idDzenLyrics: String get() = fields[SongField.ID_DZEN_LYRICS]?.nullIfEmpty() ?: ""
    val idDzenKaraoke: String get() = fields[SongField.ID_DZEN_KARAOKE]?.nullIfEmpty() ?: ""
    val idDzenChords: String get() = fields[SongField.ID_DZEN_CHORDS]?.nullIfEmpty() ?: ""
    val idDzenMelody: String get() = fields[SongField.ID_DZEN_MELODY]?.nullIfEmpty() ?: ""
    val versionDzenLyrics: Int get() = (fields[SongField.VERSION_DZEN_LYRICS]?.nullIfEmpty() ?: "0").toInt()
    val versionDzenKaraoke: Int get() = (fields[SongField.VERSION_DZEN_KARAOKE]?.nullIfEmpty() ?: "0").toInt()
    val versionDzenChords: Int get() = (fields[SongField.VERSION_DZEN_CHORDS]?.nullIfEmpty() ?: "0").toInt()
    val versionDzenMelody: Int get() = (fields[SongField.VERSION_DZEN_MELODY]?.nullIfEmpty() ?: "0").toInt()

    val idVkLyrics: String get() = fields[SongField.ID_VK_LYRICS]?.nullIfEmpty() ?: ""
    ...
    val idVkKaraoke: String get() = fields[SongField.ID_VK_KARAOKE]?.nullIfEmpty() ?: ""
    ...
    val idVkChords: String get() = fields[SongField.ID_VK_CHORDS]?.nullIfEmpty() ?: ""
    val idVkMelody: String get() = fields[SongField.ID_VK_MELODY]?.nullIfEmpty() ?: ""
    ...
    val versionVkLyrics: Int get() = (fields[SongField.VERSION_VK_LYRICS]?.nullIfEmpty() ?: "0").toInt()
    val versionVkKaraoke: Int get() = (fields[SongField.VERSION_VK_KARAOKE]?.nullIfEmpty() ?: "0").toInt()
    val versionVkChords: Int get() = (fields[SongField.VERSION_VK_CHORDS]?.nullIfEmpty() ?: "0").toInt()
    val versionVkMelody: Int get() = (fields[SongField.VERSION_VK_MELODY]?.nullIfEmpty() ?: "0").toInt()

    val idTelegramLyrics: String get() = fields[SongField.ID_TELEGRAM_LYRICS]?.nullIfEmpty() ?: ""
    val idTelegramKaraoke: String get() = fields[SongField.ID_TELEGRAM_KARAOKE]?.nullIfEmpty() ?: ""
    val idTelegramChords: String get() = fields[SongField.ID_TELEGRAM_CHORDS]?.nullIfEmpty() ?: ""
    val idTelegramMelody: String get() = fields[SongField.ID_TELEGRAM_MELODY]?.nullIfEmpty() ?: ""

    val versionTelegramLyrics: Int get() = (fields[SongField.VERSION_TELEGRAM_LYRICS]?.nullIfEmpty() ?: "0").toInt()
    val versionTelegramKaraoke: Int get() = (fields[SongField.VERSION_TELEGRAM_KARAOKE]?.nullIfEmpty() ?: "0").toInt()
    val versionTelegramChords: Int get() = (fields[SongField.VERSION_TELEGRAM_CHORDS]?.nullIfEmpty() ?: "0").toInt()
    val versionTelegramMelody: Int get() = (fields[SongField.VERSION_TELEGRAM_MELODY]?.nullIfEmpty() ?: "0").toInt()
    ...
    fun telegramIdSettingField(songVersion: SongVersion): SongField =
        when (songVersion) {
            SongVersion.LYRICS -> SongField.ID_TELEGRAM_LYRICS
            SongVersion.KARAOKE -> SongField.ID_TELEGRAM_KARAOKE
            SongVersion.CHORDS -> SongField.ID_TELEGRAM_CHORDS
            SongVersion.TABS -> SongField.ID_TELEGRAM_MELODY
        }
    ...
    val idPlLyrics: String get() = fields[SongField.ID_PL_LYRICS]?.nullIfEmpty() ?: ""
    val idPlKaraoke: String get() = fields[SongField.ID_PL_KARAOKE]?.nullIfEmpty() ?: ""
    val idPlChords: String get() = fields[SongField.ID_PL_CHORDS]?.nullIfEmpty() ?: ""
    val idPlMelody: String get() = fields[SongField.ID_PL_MELODY]?.nullIfEmpty() ?: ""

    val versionPlLyrics: Int get() = (fields[SongField.VERSION_PL_LYRICS]?.nullIfEmpty() ?: "0").toInt()
    val versionPlKaraoke: Int get() = (fields[SongField.VERSION_PL_KARAOKE]?.nullIfEmpty() ?: "0").toInt()
    val versionPlChords: Int get() = (fields[SongField.VERSION_PL_CHORDS]?.nullIfEmpty() ?: "0").toInt()
    val versionPlMelody: Int get() = (fields[SongField.VERSION_PL_MELODY]?.nullIfEmpty() ?: "0").toInt()

    val idMaxLyrics: String get() = fields[SongField.ID_MAX_LYRICS]?.nullIfEmpty() ?: ""
    val idMaxKaraoke: String get() = fields[SongField.ID_MAX_KARAOKE]?.nullIfEmpty() ?: ""
    val idMaxChords: String get() = fields[SongField.ID_MAX_CHORDS]?.nullIfEmpty() ?: ""
    val idMaxMelody: String get() = fields[SongField.ID_MAX_MELODY]?.nullIfEmpty() ?: ""

    val versionMaxLyrics: Int get() = (fields[SongField.VERSION_MAX_LYRICS]?.nullIfEmpty() ?: "0").toInt()
    val versionMaxKaraoke: Int get() = (fields[SongField.VERSION_MAX_KARAOKE]?.nullIfEmpty() ?: "0").toInt()
    val versionMaxChords: Int get() = (fields[SongField.VERSION_MAX_CHORDS]?.nullIfEmpty() ?: "0").toInt()
    val versionMaxMelody: Int get() = (fields[SongField.VERSION_MAX_MELODY]?.nullIfEmpty() ?: "0").toInt()

    // Публикация DEMO-версии (RenderVersion.DEMO) — независимо от SongVersion, см. karaoke-app CLAUDE.md/архитектурные заметки.
    val idDzenDemo: String get() = fields[SongField.ID_DZEN_DEMO]?.nullIfEmpty() ?: ""
    val versionDzenDemo: Int get() = (fields[SongField.VERSION_DZEN_DEMO]?.nullIfEmpty() ?: "0").toInt()
    val idVkDemo: String get() = fields[SongField.ID_VK_DEMO]?.nullIfEmpty() ?: ""
    val versionVkDemo: Int get() = (fields[SongField.VERSION_VK_DEMO]?.nullIfEmpty() ?: "0").toInt()
    val idTelegramDemo: String get() = fields[SongField.ID_TELEGRAM_DEMO]?.nullIfEmpty() ?: ""
    ...
    val indexTabsVariant: Int get() = (fields[SongField.INDEX_TABS_VARIANT]?.nullIfEmpty() ?: "0").toInt()

    val rate: Int get() = (fields[SongField.RATE]?.nullIfEmpty() ?: "0").toInt()
    ...
    var idTariff: Int
        get() = (fields[SongField.ID_TARIFF]?.nullIfEmpty() ?: "0").toInt()
        set(value) {
            fields[SongField.ID_TARIFF] = value.toString()
        }

    // Тип песни: песня (вокал+музыка), инструментал (только музыка), стихи (только вокал).
    // Хранится в tbl_songs.song_type в lowercase (song/instrumental/poetry) — сравниваем через
    ...
    var songType: SongType
        get() {
            val raw = fields[SongField.SONG_TYPE]?.nullIfEmpty() ?: SongType.SONG.dbValue
            return SongType.entries.firstOrNull { it.dbValue == raw.lowercase() } ?: SongType.SONG
        }
        set(value) {
            fields[SongField.SONG_TYPE] = value.dbValue
        }
    ...
    fun argsDemucs2(
        ...
        // потом копируем оттуда результат. Финальный шаг (executeFinalizeDemucs) сам удаляет
        // PATH_TO_TEMP_DEMUCS_FOLDER — очередь заданий не прерывает цепочку шагов при ошибке одного из
        // них (см. KaraokeProcessThread.run()/getProcessesToStart), поэтому только финальный шаг может
        // достоверно проверить, что demucs реально создал оба стема, и при необходимости поставить в
        // очередь повтор без GPU (argsDemucs2RetryCpu) вместо молчаливого "успеха".
    ...
    fun argsDemucs5(
        ...
        // Сначала копируем файл аудио в папку PATH_TO_TEMP_DEMUCS_FOLDER с именем file.flac, потом вызываем докер,
        // потом копируем оттуда результат. См. комментарий в argsDemucs2 про executeFinalizeDemucs/CPU-повтор.
    ...
    /**
     * Повторная попытка demucs2-шага БЕЗ GPU — вызывается из executeFinalizeDemucs (Utils.kt), когда
     * GPU-попытка (argsDemucs2, device="cuda") не досоздала часть стемов. По образцу
     * argsStemJobDemucsRetryCpu (StemJobProcessing.kt) — на админской машине GPU общий с локальной
     * LLM-моделью (не наше собственное задание из очереди), типичная причина: CUDA OOM. Шаги
     * mkdir/chmod/cp не повторяются — file.flac уже скопирован первой попыткой (это и проверяет
     * executeFinalizeDemucs перед тем, как поставить этот повтор в очередь).
     */
    fun argsDemucs2RetryCpu(threadId: Int = 1): Pair<List<List<String>>, Map<String, String>> {
        val tmpFileName = "file"
        val cpuFlags = dockerCpusFlag(cpuLimitPercentForType(KaraokeProcessTypes.DEMUCS2))
        return Pair(
            listOf(
                listOf("docker", "run", "--rm", "-i", "--name=demucs") + cpuFlags +
                    listOf(
                        "-v",
                        "$PATH_TO_TEMP_DEMUCS_FOLDER:/data/input",
                        "-v",
                        "$PATH_TO_TEMP_DEMUCS_FOLDER:/data/output",
                        "svoemestodev/demucs:latest",
                        "''./demucs2 -file $PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName.flac -recode flac -device cpu''",
                    ),
                listOf("mv", "$PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName-accompaniment.flac", accompanimentNameFlac.rightFileName()),
                listOf("chmod", "666", accompanimentNameFlac.rightFileName()),
                listOf("mv", "$PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName-vocals.flac", vocalsNameFlac.rightFileName()),
    ...
    /**
     * Повторная попытка demucs5-шага БЕЗ GPU — см. argsDemucs2RetryCpu.
     */
    fun argsDemucs5RetryCpu(threadId: Int = 1): Pair<List<List<String>>, Map<String, String>> {
        val tmpFileName = "file"
        val cpuFlags = dockerCpusFlag(cpuLimitPercentForType(KaraokeProcessTypes.DEMUCS5))
        return Pair(
            listOf(
                listOf("docker", "run", "--rm", "-i", "--name=demucs") + cpuFlags +
                    listOf(
                        ...
                        "$PATH_TO_TEMP_DEMUCS_FOLDER:/data/output",
                        "svoemestodev/demucs:latest",
                        "''./demucs5 -file $PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName.flac -recode flac -device cpu''",
                    ),
                listOf("mv", "$PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName-accompaniment.flac", accompanimentNameFlac.rightFileName()),
                listOf("chmod", "666", accompanimentNameFlac.rightFileName()),
                listOf("mv", "$PATH_TO_TEMP_DEMUCS_FOLDER/$tmpFileName-vocals.flac", vocalsNameFlac.rightFileName()),
    ...
    val sheetsageInfo: Map<String, Any> get() {
        // key - тональность
        // tempo - bpm
        ...
        if (matchResultChords != null) {
            ...
            chords.forEach { chordText ->
                ...
                if (matchResultChord != null) {
                    ...
                    for (@Suppress("unused") i in 0 until beats) {
                        indexBeat++
    ...
    @get:JsonIgnore
    var voicesForMlt: List<SongVoice> = emptyList()
...
//        mltProp.setFadeLengthMs(fadeMs)
        ...
        // setBackgroundEndTimecode - таймкод конца фона (общая длительность)
...
//        mltProp.setLengthFr(convertMillisecondsToFrames(timelineLengthMs, Karaoke.frameFps), "Total")
    ...
    fun setIndexTabsVariant(ind: Int) {
        fields[SongField.INDEX_TABS_VARIANT] = ind.toString()
        saveToDb()
    }
    ...
    fun getStringsForAllNotesInSong(voice: Int): List<List<DiffGuitarStringLad>> {
        ...
        if (markersNotes.isEmpty()) return emptyList()
    ...
    fun getFormattedChords(): String {
        ...
        for (voice in 0 until countVoices) {
            ...
            if (hasChords) {
                ...
                val spanStyleChord = """<span style="color: #00BFFF; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
                val spanStyleText = """<span style="color: #FFFFFF; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
                val spanEnd = """</span>"""
                ...
                if (capo > 0) {
                    ...
                    var lineCapo = spanStyleCapo + "Каподастр на $capo-м ладу" + spanEnd + br
                    lineCapo += spanStyleCapo + "Оригинальная тональность: $originalKey" + spanEnd + br
                    lineCapo += spanStyleCapo + "Аккорды для тональности: $newKey" + spanEnd + br
                    result += lineCapo + br
                }
                    ...
                    when (marker.markertype) {
                        ...
                        Markertype.SYLLABLES.value, Markertype.CHORD.value -> {
                            ...
                            if (marker.chord.isNotEmpty()) {
                                ...
                                var newIndexNote = MusicNote.entries.indexOf(musicNote!!) - capo
...
//                                    chord = " ".repeat(txt.length)
                            ...
                            lineChords += spanStyleChord + chordHtml + spanEnd
                            lineText += spanStyleText + txtHtml + spanEnd
    ...
    fun getFormattedNotes(): String {
        ...
        for (voice in 0 until countVoices) {
            ...
            val spanStyleNote = """<span style="color: #00BFFF; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
            val spanStyleTabLine = """<span style="color: #66BFFF; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
            val spanStyleText = """<span style="color: #FFFFFF; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
            val spanStyleOctave = """<span style="color: #444444; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
            val spanStyleDefis = """<span style="color: #666666; font-family: monospace; font-size: 15px; font-style: normal; font-weight: bolder;">"""
            val spanEnd = """</span>"""
            val br = """<br>"""
            ...
            markers.forEach { marker ->
                when (marker.markertype) {
                    ...
                    Markertype.SYLLABLES.value, Markertype.NOTE.value -> {
                        ...
                        lineText += spanStyleText + txtHtml + spanEnd
                        if (!endOfWord) {
                            lineText += "$spanStyleDefis-$spanEnd"
                        }
...
//        val SPAN_STYLE_COMMENT = """<span style="color: #D2691E; font-size: small; font-style: italic; font-weight: bolder;">"""
    ...
    fun convertSrtToMarkers(srt: String): List<SourceMarker> {
        val listSubtitleFileElements: MutableList<SongRenderContext.SubtitleFileElement> = mutableListOf()
...
//                val isEndOfLine =  (text.uppercase().startsWith("[SETTING]|") || text.endsWith("\\\\"))
                listSubtitleFileElements.add(
                    SongRenderContext.SubtitleFileElement(
                        startFrame,
                        endFrame,
                        text,
                        false,
     ...
     *    генерируется `UPDATE tbl_songs SET ...` только для изменённых полей.
     *
                ...
                diff.filter { it.recordDiffRealField }.forEach {
                    try {
                        when (it.recordDiffValueNew) {
                            // album_id — единственное nullable-поле в этой сущности (настоящий FK,
                            // ON DELETE SET NULL) — без этой ветки null записался бы как строка "null"
                            // (Any?.toString()) и упал бы на типе/FK-констрейнте колонки.
                            null -> ps.setNull(index, java.sql.Types.INTEGER)
...
//    }
    fun deleteFromDb(
        ...
        val sql = "DELETE FROM tbl_songs${if (sync) "_sync" else ""} WHERE id = ?"
        val ps = connection.prepareStatement(sql)
        ps.setLong(1, id)
        ps.executeUpdate()
        ps.close()
    ...
    fun createKaraoke(songVersion: SongVersion) {
        when (songVersion) {
...
//        if (createMelodyVk) createKaraoke(this, SongVersion.TABSVK)
    ...
    fun getSqlToInsert(sync: Boolean): String {
        ...
        return "INSERT INTO tbl_songs${if (sync) "_sync" else ""} (${fieldsValues.joinToString(
            ", ",
...
//                if (Paths.get(settNewVersion.getOutputFilename(SongOutputFile.PICTUREBOOSTYTEASER)).toFile().exists()) createBoostyTeaserPicture(settNewVersion)
        ...
        fun getDiff(
            ...
            if (settA != null && settB != null) {
                ...
                if (settA.albumId != settB.albumId) result.add(RecordDiff("album_id", settA.albumId, settB.albumId))
        ...
        fun getLastUpdated(
            ...
            try {
                statement = connection.createStatement()
                sql = "SELECT id FROM tbl_songs WHERE last_update > '${Timestamp(lastTime)}'::timestamp"
                rs = statement.executeQuery(sql)
                while (rs.next()) {
        ...
        fun createDbInstance(
            ...
            try {
                ...
                val rsLastId = statement.executeQuery("select max(id) as last_value from tbl_songs;")
                val rsLastSeq = statement.executeQuery("select last_value from tbl_songs_id_seq;")
                rsLastId.next()
                val lastId = rsLastId.getLong("last_value")
                rsLastSeq.next()
                val lastSeq = rsLastSeq.getLong("last_value")
                if (lastSeq < lastId) {
                    statement.execute("alter sequence tbl_songs_id_seq restart with ${lastId + 1};")
                    ps.executeUpdate(sql, Statement.RETURN_GENERATED_KEYS)
                }
            }
        ...
        fun loadListAuthors(
            ...
            try {
                ...
                sql =
                    when {
                        // from tbl_songs (с skip-фильтром по умолчанию); опционально только спецзаказные
                        withSkiped && isSpecialOrder == null -> "select DISTINCT song_author from tbl_songs order by song_author"
                        withSkiped && isSpecialOrder == true ->
                            "select DISTINCT song_author from tbl_songs where song_author in (select author from tbl_authors where is_special_order = true) order by song_author"
                        withSkiped && isSpecialOrder == false ->
                            "select DISTINCT song_author from tbl_songs where song_author in (select author from tbl_authors where is_special_order = false) order by song_author"
        ...
        fun loadListAlbums(database: KaraokeConnection): List<String> {
            ...
            try {
                statement = connection.createStatement()
                sql = "select DISTINCT song_album from tbl_songs order by song_album"
        ...
        fun listHashes(
            ...
            val sql = "SELECT id, recordhash FROM tbl_songs $whereText"
         ...
         *
         * Алгоритм:
         * 1. Формирует SQL `SELECT ... FROM tbl_songs` с `WHERE`-условиями из [args]
         *    (например, `"author" -> "Ундервуд"`, `"year" -> "2024"`, `"text" -> "любовь"`).
         * 2. Выполняет запрос пакетно (НЕ N+1), сохраняет результат в `List<Song>`.
                ...
                if (args.containsKey("sort")) {
                    when (args["sort"]) {
                        ...
                        "author" -> return result.sortedWith(
                            compareBy<Song> { it.author }.thenBy { it.dateTimePublish }.thenBy { it.id },
                        )
                    }
                }
        ...
        fun loadFromFile(
            ...
            body.split("\n").forEach { line ->
                ...
                if (settingList.size > 1) {
                    ...
                    val settingField =
                        if (SongField.entries.map { it.name }.contains(
                                settingName,
                            )
                        ) {
                            SongField.valueOf(settingName)
                        } else {
                            null
                        }
...
//                            )
        ...
        fun getSetOfTags(database: KaraokeConnection): Set<String> {
            ...
            try {
                statement = connection.createStatement()
                sql = "select DISTINCT tbl_songs.tags as tags " +
                    "from tbl_songs " +
    ...
    override fun compareTo(other: Song): Int = sortString.compareTo(other.sortString)

    override fun toDTO(): SongDTO =
        SongDTO(
            id = id,
            idPrevious = id,
            idNext = id,
            ...
            albumName =
                albumId?.let { aid ->
                    Album.getAlbumById(aid, database = database, storageService = storageService, storageApiClient = storageApiClient)?.name
                } ?: "",
        )

    fun copyFieldsFromAnother(
        idAnother: Long,
        listFields: List<SongField>,
    ) {
        loadFromDbById(
            id = idAnother,
