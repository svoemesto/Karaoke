...
- [X] T007 В `karaoke-web/src/main/kotlin/com/svoemesto/karaokeweb/controllers/MainController.kt`, метод `doRegisterEvent`, ветка `EventType.PLAYER.dbValue` при `linkType == PlayerAction.PLAY.dbValue`: вызов `ListeningHistory.upsert(siteUserId, songId, WORKING_DATABASE)` **дополнительно** к существующему `insertEvent(...)` (не вместо), только если `siteUserId > 0` (см. `research.md` Decision 4 — не трогаем существующую запись в `tbl_events`) *(ИСПРАВЛЕНО 2026-07-25, два независимых бага подряд:*
*1) изначально было ошибочно повешено на `EventType.PLAY.dbValue` — это legacy-событие клика по внешней VK-ссылке на странице песни, **не** реальный запуск онлайн-плеера (`EventTypes.kt`: "PLAY(\"play\"), // legacy... НЕ онлайн-плеер"). Реальное прослушивание шлёт `KaraokePlayer.js` → `trackPlayerPlay()` → `eventType='player', linkType='play'`. Исправлено переносом вызова в правильную ветку.*
*2) После переноса админ по-прежнему не видел записей. Диагностика через DevTools (Network) показала: браузер шлёт POST `/api/public/events` с `Authorization: Bearer ...`, но `Content-Type: text/plain;charset=UTF-8` вместо `application/x-www-form-urlencoded` → Spring не может распарсить `@RequestParam Map<String,Any> data` из тела → `data` пустая → любое трекинг-событие залогиненного пользователя (не только play — вообще любой вызов `apiPost()` с auth-заголовком) тихо отбрасывалось на первой же строке `doRegisterEvent`, отвечая `{"ok":false}` без исключений и логов. Корень — `karaoke-public/src/lib/utils.js: promisedXMLHttpRequest()`: дефолтный `Content-Type` ставился только когда `obj.headers === undefined` целиком, а `apiPost()` всегда передаёт объект headers (пусть даже `authHeader()` вернул `undefined` только для анонима) — для залогиненных `obj.headers` — непустой объект с `Authorization`, дефолт пропускался, браузер сам подставлял `text/plain` для `xhr.send(string)` без явного Content-Type. Исправлено: дефолтный Content-Type теперь мержится всегда, а не только когда headers целиком не задан. Это баг сайтового масштаба — ломал ЛЮБОЙ POST через `apiPost()`/`promisedXMLHttpRequest` для залогиненных пользователей (все `trackXxx()` в `tracking.js`, вероятно и другие вызовы через `authApi.js`/`playlistApi.js`), не только слушание.*
*Оба фикса проверены вживую: пересобраны/перезапущены `karaoke-web` и `karaoke-public`; воспроизведён и подтверждён баг №2 прямым curl-сравнением (`text/plain` → `{"ok":false}`, `application/x-www-form-urlencoded` → `{"ok":true}`, идентично отчёту админа); реальный POST `/api/public/events` с `eventType=player&linkType=play` и валидным токеном → строка появляется; повтор увеличивает `play_count`/обновляет `last_played_at` без дублирования; `linkType=pause` не апсертит; чтение через `/api/public/account/history` подтверждено; тестовые данные удалены)*
