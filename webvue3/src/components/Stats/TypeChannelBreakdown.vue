<template>
  <div class="breakdown-row">
    <div class="chart-card">
      <h6 class="chart-title">Типы событий <span class="hint">(клик по сегменту — детализация)</span></h6>
      <div v-if="isLoading" class="text-center py-4"><BSpinner small /></div>
      <apexchart v-else-if="typeSeries.length" type="donut" height="300" :options="typeOptions" :series="typeSeries" />
      <div v-else class="text-muted text-center py-4">Нет данных</div>
    </div>
    <div class="chart-card">
      <h6 class="chart-title">Каналы переходов</h6>
      <div v-if="isLoading" class="text-center py-4"><BSpinner small /></div>
      <apexchart v-else-if="channelSeries[0].data.length" type="bar" height="300" :options="channelOptions" :series="channelSeries" />
      <div v-else class="text-muted text-center py-4">Нет данных</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    typeOptions() {
      return {
        chart: { events: { dataPointSelection: (e, ctx, cfg) => this.onDonutClick(cfg.dataPointIndex) } },
        labels: this.byType.map(i => typeLabel(i.name)),
        colors: this.byType.map(i => typeColor(i.name)),
        legend: { position: 'bottom' },
        dataLabels: { enabled: true, formatter: (val) => `${val.toFixed(1)}%` },
        plotOptions: { pie: { donut: { labels: { show: true, total: { show: true, label: 'Всего', formatter: (w) => w.globals.seriesTotals.reduce((a, b) => a + b, 0).toLocaleString('ru-RU') } } } } },
...
.chart-title { margin: 0 0 8px; font-size: 0.95rem; font-weight: 600; }
.hint { font-weight: 400; font-size: 0.75rem; color: #999; }
</style>
