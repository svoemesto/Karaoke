...
export default {
  ...
  computed: {
    ...
    isSavingRowsPerPage() {
      return this.$store.getters.isSavingRowsPerPage('dictionaries')
    },
  ...
  async mounted() {
    ...
    this.perPage = this.$store.getters.getRowsPerPage('dictionaries')
  ...
  methods: {
    ...
    async onPerPageChange(e) {
      ...
      if (isNaN(parsed) || parsed < 1 || parsed > 1000) {
        ...
        console.warn('[Dictionaries.onPerPageChange] invalid value', rawValue)
        return
      }
      if (parsed === this.perPage) return
      this.currentPage = 1 // ADR-0004: page reset
      if (this.$store.getters.getDictionariesTableCurrentPage !== undefined) {
        this.$store.commit('setDictionariesTableCurrentPage', 1)
      }
      const ok = await this.$store.dispatch('setRowsPerPage', {
        tableKey: 'dictionaries',
        value: parsed,
      })
      if (ok) {
        this.perPage = this.$store.getters.getRowsPerPage('dictionaries')
        await this.$store.dispatch('loadDictionariesDigests', {
          page: this.currentPage,
          perPage: this.perPage,
          ...this.$store.getters.getDictionariesFilter,
        })
      }
    },
    create() {
...
.dictionaries-bv-table-header .pagination,
.dictionaries-bv-table-header ul.pagination {
