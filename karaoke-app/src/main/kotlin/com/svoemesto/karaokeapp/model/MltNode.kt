...
data class MltNode(
    ...
    override fun toString(): String =
        "${if (comment == "") "" else "<!-- $comment -->\n"}<$name ${fields.map { "${it.key}=\"${it.value}\"" }.joinToString(
            " ",
        )}${if (body == null) {
            "/>"
        } else {
...
data class MltNodeBuilder(
    ...
    fun node(node: MltNode) =
        apply {
            nodes.add(node)
        }

    fun nodes(nodesToAdd: List<MltNode>) =
        apply {
            nodes.addAll(nodesToAdd)
        }

    fun startviewport(rect: String) =
        apply {
            nodes.add(
                MltNode(
                    name = "startviewport",
    ...
    fun filterAudiolevel(name: String) =
        apply {
            nodes.add(
                MltNode(
                    name = "filter",
...
//    fun hideByType(type: ProducerType) = apply { properties["hide"] = if (!type.isAudio) "audio" else "video" }
    fun hideByType(type: ProducerType) =
        apply {
            properties["hide"] =
                if (type == ProducerType.SCROLLERS) {
                    "both"
    ...
    fun build(): MutableMap<String, String> = properties
}
