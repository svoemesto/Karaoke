...
- [X] T053 [P] Перегенерировать `deploy/new_comp/sm-karaoke-system/dumps/karaoke_clear_dump.sql` под новые имена таблиц — **сделано** через `pg_dump --create --schema-only --no-owner --no-privileges` (та же команда, что и `deploy/create_clear_db.sh`) на актуальной схеме; проверено восстановлением во временную БД (`karaoke_dumptest`) без ошибок — `tbl_songs`/`tbl_songs_sync`/`tbl_albums`/`tbl_song_authors` присутствуют, `tbl_settings` не осталось. См. GitHub issue #66.
...
- [X] T055 Валидация выполнена в два этапа: в песочнице — через прямые API-вызовы (бэкфилл: 2162 группы/альбома, 19441 песня, идемпотентность подтверждена повторным запуском; Album CRUD; FR-008 — отклонение чужого альбома; US2 edge cases — главный автор отклонён, дубликат идемпотентен; `/api/sync/entities` — albums/songcoauthors зарегистрированы, `key=settings` сохранён; FR-007 — живое изменение sortOrder меняет порядок на `/api/public/zakroma`); затем администратор самостоятельно проверил и применил изменения на LOCAL и на PROD через реальный UI — всё работает (подтверждено 2026-07-27).
