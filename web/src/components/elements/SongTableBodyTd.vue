<template>
  <div :style="styleRoot">
    <div :style="styleVal" @dblclick="$emit('dblclick')">
      {{song[this.name]}}
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    textAlign() {
      return this.$store.getters.getFieldParams(this.name).textAlign;
    },
    fontSize() {
      return this.$store.getters.getFieldParams(this.name).fontSize;
    },
    width() {
    ...
    styleRoot() {
      let result = {
        ...
        minWidth: this.width,
        maxWidth: this.width,
        borderStyle: `solid solid ${this.isCurrent || this.isLast ? 'solid' : 'none'} ${this.position === 'left' ? 'solid' : 'none'}`,
