...
object PlayerMp4MuxService {
    ...
    data class AudioTrack(val flacPath: String, val gain: Double)

    /** Состав/громкости по умолчанию — как у MLT-версии Karaoke: аккомпанемент слышен, вокал приглушён. */
    fun defaultTracks(settings: Settings): List<AudioTrack> = listOf(
    ...
    fun mixAndMux(
        ...
        val args = mutableListOf(
            ...
            "-i", "${framesDir.absolutePath}/frame_%06d.jpg",
        )
        audioTracks.forEach { args.add("-i"); args.add(it.flacPath) }
        args.addAll(
            listOf(
