  ...
  <div class="km-page">
    ...
    <div v-else class="km-content">
      ...
      <div v-else class="km-list">
        <div v-for="pl in playlists" :key="pl.id" class="km-pl-card" :class="{ 'km-pl-fav': pl.favorites }">
        ...
        <p v-if="!playlists.length" class="km-empty">Пока нет плейлистов. Добавьте песни в «Избранное» кнопкой-закладкой в таблицах.</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    open(id) { this.$router.push({ path: `/account/playlists/${id}` }) },
    async onCreate() {
      if (!this.isPremium) {
...
.km-head-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; gap: 1rem; }
.km-title { font-size: 1.4rem; margin: 0; }
.km-new-btn {
  background: var(--km-accent); color: #fff; border: none; border-radius: 8px;
  padding: 0.5rem 1rem; font-size: 0.88rem; font-weight: 600; cursor: pointer; white-space: nowrap;
}
.km-new-btn:hover { opacity: 0.9; }
.km-new-btn:disabled { opacity: 0.6; cursor: default; }
.km-loading { padding: 2rem; text-align: center; color: var(--km-text2); }
.km-empty { color: var(--km-text2); font-size: 0.9rem; padding: 1rem 0; }

.km-list { display: flex; flex-direction: column; gap: 0.6rem; }
.km-pl-card {
  display: flex; align-items: center; gap: 0.85rem;
  background: var(--km-card); border: 1px solid var(--km-border); border-radius: 12px;
  padding: 0.75rem 1rem;
}
.km-pl-fav { border-color: #e6a3ab; }
.km-pl-icon { flex-shrink: 0; line-height: 0; }
.km-pl-main { flex: 1; cursor: pointer; min-width: 0; }
.km-pl-name { font-size: 1rem; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.km-pl-count { font-size: 0.78rem; color: var(--km-text2); }
.km-pl-actions { display: flex; gap: 0.4rem; flex-shrink: 0; }
.km-pl-btn {
...
.km-pl-btn:hover { background: var(--km-hover); color: var(--km-text); }
.km-pl-btn-danger:hover { border-color: #d02c3a; color: #d02c3a; }
.km-pl-btn:disabled { opacity: 0.5; cursor: default; }
</style>
