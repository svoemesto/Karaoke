      ...
      <div v-else class="km-sub-list">
        <div v-for="s in subscriptions" :key="s.id" class="km-sub-card">
          ...
          <div class="km-sub-actions">
            <RouterLink v-if="s.scope === 'SONG' && s.status === 'PAID'" :to="`/song?id=${s.idSong}`" class="km-btn km-btn-secondary">Играть</RouterLink>
            <template v-if="s.scope === 'SITE' && s.status === 'PAID'">
              <span v-if="!s.autoRenew" class="km-sub-canceled">Автопродление отключено</span>
              <button v-else class="km-btn km-btn-danger" @click="onCancel(s.id)">Отключить автопродление</button>
            </template>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    statusText(status) {
      return { CREATED: 'создана', PENDING: 'ожидает оплаты', PAID: 'активна', FAILED: 'не оплачена', REFUNDED: 'возврат', CANCELED: 'отменена' }[status] || status
    },
    async load() {
      this.loading = true
      try {
    ...
    async onCancel(id) {
      if (!confirm('Отключить автопродление? Доступ сохранится до конца оплаченного периода.')) return
...
.km-sub-list { display: flex; flex-direction: column; gap: 0.75rem; }
.km-sub-card {
  display: flex; justify-content: space-between; align-items: center; gap: 1rem;
  background: var(--km-card); border: 1px solid var(--km-border); border-radius: 12px; padding: 1rem 1.25rem;
}
.km-sub-title { font-weight: 600; font-size: 0.95rem; }
.km-sub-meta { font-size: 0.8rem; color: var(--km-text2); margin-top: 0.2rem; }
.km-sub-actions { display: flex; align-items: center; gap: 0.5rem; flex-shrink: 0; }
.km-sub-canceled { font-size: 0.78rem; color: var(--km-text2); }
.km-btn { display: inline-block; border-radius: 8px; padding: 0.4rem 0.9rem; font-weight: 600; text-decoration: none; cursor: pointer; border: none; font-size: 0.82rem; white-space: nowrap; }
