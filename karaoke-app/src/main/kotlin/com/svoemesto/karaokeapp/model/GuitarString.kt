...
fun GuitarString.getNotes(): List<NoteOctaveFret> = (0 until countFrets).map { fret -> getNote(fret) }.toList()
...
fun GuitarString.getFrets(note: MusicNote): List<NoteOctaveFret> = getNotes().filter { it.musicNote == note }.toList()
...
fun GuitarString.getPrintedString(notes: List<MusicNote>): String {
    ...
    notes
        .flatMap { note ->
            getFrets(note).map { nof -> Pair(nof.fret, notes.indexOf(note)) }
        }.forEach { fretAndIndex ->
            strString =
                strString.replaceRange(fretAndIndex.first * 6 + 3, fretAndIndex.first * 6 + 4, fretAndIndex.second.toString())
        }
    return strString
}
