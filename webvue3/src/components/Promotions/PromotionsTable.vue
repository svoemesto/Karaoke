  ...
  <div class="prm-wrapper">
    ...
    <div class="prm-hint">
      ...
      <ul>
        ...
        <li>
          <b>HAPPY_HOUR</b>: {"percent":15,"hours":[20,21,22],"daysOfWeek":[5,6,7]} (часы 0-23, дни
          ISO 1=Пн..7=Вс)
        </li>
        <li><b>FLAT_PERCENT</b>: {"percent":10}</li>
        <li>
          <b>CART_BULK_PERCENT</b>: {"minQty":10,"percent":30} — скидка на ВЕСЬ заказ «Корзины»,
          если товаров в нём ≥ minQty (применяется к SONG, количество считается строго внутри одного
          заказа, в отличие от NTH_FREE, который считает по всем покупкам пользователя за всё время)
        </li>
      </ul>
    </div>
      ...
      <tbody>
        <tr v-for="item in editable" :key="item.id">
          ...
          <td class="prm-actions">
            <button class="prm-btn" :disabled="!isChanged(item)" @click="save(item)">
              Сохранить
            </button>
...
export default {
  ...
  computed: {
    isLoading() {
      return this.$store.getters.getPromoRulesIsLoading
    },
    list() {
      return this.$store.getters.getPromoRulesList
    },
    target: {
      get() {
        return this.$store.getters.getPromoRulesTarget
      },
      set(v) {
        this.$store.dispatch('setPromoRulesTarget', v)
      },
    },
    canCreate() {
      return this.newItem.name.trim() !== '' && this.isValidJson(this.newItem.paramsJson)
    },
  },
  watch: {
    list: {
      ...
      handler(list) {
        ...
        this.original = {}
        list.forEach((i) => {
          this.original[i.id] = Object.assign({}, i)
        })
      },
    },
  },
  ...
  methods: {
    ...
    async save(item) {
      if (!this.isValidJson(item.paramsJson)) {
