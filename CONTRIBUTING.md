...
**Severity**: MUST
**Section**: tooling
**Enforced by**: CI (отдельная стадия)

CI **обязательно** запускает `./gradlew ktlintCheck detekt`,
`cd webvue3 && npm run lint:check`, `cd karaoke-public && npm run lint:check`,
`./tools/baseline-stats.sh`. Любое увеличение baseline-метрик (новые
нарушения сверх baseline) — блокер.

**Связанные инварианты**: [`specs/001-code-standards-docs/spec.md#sc-002`](../specs/001-code-standards-docs/spec.md)
