      ...
      <div v-else class="km-news-list">
        ...
        >
          <div class="km-news-card-header">
            <span class="km-news-badge" :class="`km-news-badge-${item.category}`">{{ categoryIcon(item.category) }} {{ categoryLabel(item.category) }}</span>
...
.km-loading, .km-empty { color: var(--km-text2); text-align: center; padding: 2rem 0; }

.km-news-list { display: flex; flex-direction: column; gap: 1rem; }
.km-news-card {
...
.km-news-card-clickable { cursor: pointer; }
.km-news-card-clickable:hover { background: var(--km-hover); }
.km-news-card-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem; }
.km-news-badge {
  font-size: 0.75rem;
  font-weight: 700;
  border-radius: 20px;
...
.km-news-badge-air { color: #0a5cbf; background: linear-gradient(135deg, #cfe6ff, #9cc9ff); }
.km-news-badge-premium { color: #7a5c00; background: linear-gradient(135deg, #ffe27a, #d4af37); }
.km-news-badge-feature { color: #5a2d8f; background: linear-gradient(135deg, #e6d4ff, #c299f2); }
.km-news-badge-general { color: var(--km-text2); background: var(--km-hover); }
.km-news-date { font-size: 0.75rem; color: var(--km-text2); white-space: nowrap; }
.km-news-title { font-size: 1.05rem; margin: 0 0 0.4rem; }
.km-news-body { font-size: 0.9rem; color: var(--km-text2); margin: 0; white-space: pre-wrap; word-break: break-word; }
.km-news-link-hint { display: inline-block; margin-top: 0.5rem; font-size: 0.85rem; color: var(--km-accent); font-weight: 600; }
</style>
