...
> **Синхронизация аудио-потомков** (#141, specs/413): статус 5 — не только
> предфинальная вычитка, но и **целевой статус потомка** после успешной
> синхронизации от родителя (см.
> [audio-descendant-sync](audio-descendant-sync.md)). Ворота запуска —
> родитель в статусе ≥ 5; переход `5→6` повторную синхронизацию не запускает.
