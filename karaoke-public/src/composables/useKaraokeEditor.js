...
// (`karaoke-player-settings`), чтобы настройки редактора и плеера не пересекались — редактор и
// плеер разные UX, у них разные ползунки и разумные дефолты.
//
// localStorage может быть недоступен (приватный режим / квота) — обе функции тихо ничего не
// делают и возвращают дефолты.
...
// Спецтеги в тексте песни (~имя~ / ~имя:значение~, только отдельной строкой) - третья независимая
// реализация того же контракта, что и backend SpecTags.kt / webvue3 SubsEdit.vue. Единственный
// источник правды: specs/010-lyrics-spec-tags/contracts/tag-registry.md - все реализации ДОЛЖНЫ
// давать идентичный результат распознавания на одном и том же тексте. Здесь особенно важно: текст
// задания (sourceText) в этом редакторе — прямая копия официального текста песни из admin-БД, то
// есть уже МОЖЕТ содержать теги, расставленные админом, даже если сам волонтёр их не печатал.
...
// после какого ординального индекса splitSyllables(sourceText) он стоит (тот же принцип, что и в
// SubsEdit.vue specTagAnchors). syllableIndex считается через сам splitSyllables на РЕАЛЬНОМ
// префиксе обработанных строк, поэтому всегда совпадает с полным результатом.
...
// (EditorWorkView.vue) сама перерисовывает регионы после вызова.
...
// строки перед/после вставки, если рядом уже нет своего "\n". Зеркало insertSpecTagAtCursorImpl из
// SubsEdit.vue. Чистая функция - DOM/textarea-специфику (курсор, фокус) держит EditorWorkView.vue.
...
// к соседям. Спецтеги на отдельных строках снимаются перед разбивкой (см. stripSpecTagOnlyLines).
