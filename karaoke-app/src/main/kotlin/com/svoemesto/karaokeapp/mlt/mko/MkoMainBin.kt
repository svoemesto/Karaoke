...
data class MkoMainBin(
    ...
    override fun trackPlaylist(): MltNode {
        ...
        for (voiceI in 0 until mltProp.getCountVoices()) {
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(ProducerType.VOICES)}.${mltProp.getId(listOf(ProducerType.VOICE, voiceI))}",
                "${ProducerType.VOICE.name}$voiceI",
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.VOICE, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.COUNTERS, voiceI))}",
                ProducerType.COUNTERS.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(listOf(ProducerType.VOICE, voiceI))}.${mltProp.getId(listOf(ProducerType.LINES, voiceI))}",
                ProducerType.LINES.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.LINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.STRING, voiceI))}",
                ProducerType.STRING.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(listOf(ProducerType.LINES, voiceI))}.${mltProp.getId(listOf(ProducerType.LINE, voiceI))}",
                ProducerType.LINE.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(listOf(ProducerType.LINES, voiceI))}.${mltProp.getId(listOf(ProducerType.FILL, voiceI))}",
                ProducerType.FILL.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.LINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.ELEMENT, voiceI))}",
                ProducerType.ELEMENT.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.LINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.CHORDS, voiceI))}",
                ProducerType.CHORDS.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.VOICE, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.CHORDPICTURELINES, voiceI))}",
                ProducerType.CHORDPICTURELINES.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.CHORDPICTURELINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.CHORDPICTUREIMAGE, voiceI))}",
                ProducerType.CHORDPICTUREIMAGE.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.CHORDPICTURELINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.CHORDPICTUREELEMENT, voiceI))}",
                ProducerType.CHORDPICTUREELEMENT.name,
            )
            foldersNodesBuilder.propertyName(
                "kdenlive:folder.${mltProp.getId(
                    listOf(ProducerType.CHORDPICTURELINES, voiceI),
                )}.${mltProp.getId(listOf(ProducerType.CHORDPICTURELINE, voiceI))}",
                        ...
                        mutableMapOf(
                            "producer" to
                                if (type.isSequence) {
                                    ...
                                    MltGenerator.nameProducer(
                                        type,
                                        Integer.max(mis.voiceId, 0),
                                        Integer.max(mis.childId, 0),
                                        Integer.max(mis.elementId, 0),
                                    )
                                },
                            "in" to "00:00:00.000",
                            "out" to (if (type == ProducerType.MAINBIN) totalEndTimecode else songEndTimecode),
                        ),
                ),
            )
        }
                    ...
                    MltNodeBuilder()
                        ...
                        .node(
                            MltNode(
                                ...
                                fields =
                                    mutableMapOf(
                                        "producer" to MltGenerator.nameProducerBlackTrack(ProducerType.MAINBIN),
                                    ),
                            ),
                        ).nodes(
                            {
                                ...
                                songVersion.producersInMainBin.forEach {
                                    result.add(
                                        MltNode(
                                            ...
                                            fields =
                                                mutableMapOf(
                                                    "producer" to MltGenerator.nameTractor(it, voiceId),
                                                ),
                                        ),
                                    )
                                }
