...
`Started ..Application`), не только с exit code обёртки. **Детали**.
...
**Особенности Docker-образов** (детали.):
...
- **Проверка доступности внешних сервисов** (durable-инвариант): `ip-api.com`/`ipapi.co`/`ipapi.is` из Docker отдают 403/502 — **не
 использовать**. Использовать `api.country.is`. Сравнение с настройкой `homeCountry` (дефолт `"RU"`; для
 сервера в Германии — `"DE"`). Страна != `homeCountry` → ВПН включён.
...
## Деплой на продакшн-сервер (<PROD_SERVER_IP>)
...
`docker exec karaoke-web env | grep <VAR>`. (Детали.)
...
через ВПН падает по EOF на тяжёлых базовых слоях — запустить `deploy_web.sh` вручную без прокси.
...
чанкование «2 оси»), ловушки (Timestamp-биндинг, nullable-колонки, `last_update useInDiff=false`)**.
...
контроллер проверяет конфликт сам, до вызова `save()`. **Детали**.
...
`1` найден новый (обновляется `requestNewSongLastSuccessAuthor`). **Детали диагностики**.
...
их напрямую. Работа через прокси: Telegram может быть недоступен — `TelegramApiClient` реализует
...
telegram-proxy.yml`, прокси-сервис), конфиг с реальным VLESS вне git —
`/sm-karaoke/system/telegram-proxy/config.json`. Фаза 2 (постинг видео в момент эфира, снимает лимит Telegram
в 100 отложенных публикаций) — спроектирована, не реализована. **Детали**.
...
`/api/monitor/{alerts,resolve,markRead,markUnread,reset}`. **Детали**.
...
- **karaoke-web Settings trap.** В `karaoke-web` **нельзя** трогать `Settings.rootFolder` / любые `*NameFlac`-
 ...
 нового karaoke-app кода рискованно в karaoke-web ( — перенос словарей Ё/Censored
 ...
 `KaraokeStorageService`/`StorageApiClient` (как в `PublicPlaylistController`), не глобалы. Детали.
- **MTU black-hole на проде.** Любой исходящий HTTPS с прод-контейнера karaoke-web на внешний хост (MinIO
 <MINIO_IP>, Yandex SmartCaptcha и т.п.) виснет из-за MTU-дропа больших TLS-пакетов. Обход: HTTP через
...
- **Прод-сервер: диск заполняется старыми Docker-образами.** `do.sh` пушит образы всегда под
 ...
 left on device`) и весь сайт падает . **Защита уже установлена — не создавать заново:** systemd-таймер
 ...
 `ssh root@<PROD_SERVER_IP> "systemctl status karaoke-docker-prune.timer"`. Если на проде снова
 ...
 **Двойной дизайн** classic/modern (`composables/useDesign.js`, выбор в localStorage; тонкие view-обёртки
 ...
 отфильтрованном — иначе каждая нажатая клавиша дёргала бы сеть. Детали.,
...
- **Поиск (`SearchView.vue`) с алиасами авторов.** У каждого автора (`tbl_authors.aliases`,
 ...
 желательна ручная перепроверка). Детали.
- **Онлайн-плеер (`/player/:id`).** Браузерный плеер, визуально идентичный MLT-рендеру. **ДВЕ намеренные копии**
 ...
 «Скорость» (без «Открыть файл»/«Экспорт аудио» — те остаются только в admin-копии). Детали., `project_player_*`, `project_webvue3_player_button`.
- **Демо-режим онлайн-плеера.** Не-премиум/анонимный посетитель при готовом контенте (не в эфире/не
 ...
 песни. Плейлисты в демо намеренно не участвуют (и так премиум-фича). Детали.
- **Регистрация/авторизация site users (`/login`, `/register`, `/account`).** Отдельная от админки система —
 ...
 уже существующих `/api/siteplaylists/*`). Детали.,
 ...
 **обязательно** на бэкенде (`PublicSongEditorController`, иначе снятие роли не отзывает доступ к уже
 ...
 в `App.vue` (`POST /api/songeditor/submittedcount`, опрос раз в 20с, по образцу бейджа чата). Детали., `project_site_user_editor_role`.
- **«Избранное» и «Плейлисты» + плейлист автора.** `tbl_site_playlists`(+`_items`); «Избранное» = плейлист с
 ...
 `.load([id])` при смене текущей песни, иначе иконки зависают в `off`. Детали., `project_song_page_favorites_playlist`.
- **«Чат с автором проекта».** Личная переписка премиум-пользователей сайта с автором — один
 ...
 ссылкой на ВНЕШНИЙ файл внутри `background-image` браузеры не подгружают). Детали.
 **Автоприветствие при первом премиуме.** `SiteUser.sendWelcomePremiumMessageIfNeeded()` (флаг
 ...
 сообщение при ближайшем продлении/повторном Sponsr-sync. Детали.
- **«Новости».** Табличная лента + ненавязчивое всплывающее уведомление о новом контенте/функционале
 ...
 шапкой, ни с уже существующим `ChatUnreadBadge`. Скрыт на `/news` и на плеере. Детали.
 ...
 **Идемпотентность повторного клика (scope=SITE):** `PublicSubscriptionController.create()` перед
 ...
 отдельные PENDING-подписки и отдельные платежи в ЮKassa. Детали., `project_site_subscription_double_click_duplicate`.
...
(<PROD_SERVER_IP>:8832) отдельно** (миграция сама на сервер не попадает):
```
