...
export default {
    ...
    getters: {
        getStemJobs(state) { return state.stemJobs },
        getStemJobsIsLoading(state) { return state.stemJobsIsLoading },
        getStemJobsTarget(state) { return state.stemJobsTarget },
    },
    mutations: {
        setStemJobs(state, result) { state.stemJobs = result },
        setStemJobsIsLoading(state, isLoading) { state.stemJobsIsLoading = isLoading },
        setStemJobsTarget(state, target) { state.stemJobsTarget = target },
        removeStemJob(state, id) { state.stemJobs = state.stemJobs.filter(j => j.id !== id) },
    ...
    actions: {
        setStemJobsTarget(ctx, target) { ctx.commit('setStemJobsTarget', target) },
        loadStemJobs(ctx) {
            const request = { method: 'POST', url: "/api/stemjobs/list", params: { target: ctx.state.stemJobsTarget } };
        ...
        stopStemJob(ctx, id) {
            const request = { method: 'POST', url: `/api/stemjobs/${id}/stop`, params: { target: ctx.state.stemJobsTarget } };
            return promisedXMLHttpRequest(request).then(() => ctx.dispatch('loadStemJobs'));
        },
        deleteStemJob(ctx, id) {
            const request = { method: 'POST', url: `/api/stemjobs/${id}/delete`, params: { target: ctx.state.stemJobsTarget } };
