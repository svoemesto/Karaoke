<template>
  <div class="chart-card">
    ...
    <apexchart v-else-if="filtered.length" type="bar" :height="chartHeight" :options="options" :series="series" />
...
function colorForLabel(label) {
  ...
  if (label.startsWith('Соцсеть') || label.startsWith('Ссылка') || label.startsWith('Клик')) return '#f28e2b'
...
export default {
  ...
  emits: ['clear-type'],
  computed: {
    selectedTypeLabel() { return typeLabel(this.selectedType) },
    filtered() {
      if (!this.selectedType) return this.items
      return this.items.filter(i => i.eventType === this.selectedType)
    },
    chartHeight() { return Math.max(220, this.filtered.length * 26 + 40) },
    series() {
...
.filter-badge {
  margin-left: 8px; font-size: 0.78rem; font-weight: 500; background: #eef4ff;
  color: #2b5fb3; padding: 2px 8px; border-radius: 12px;
}
.reset-btn {
  border: none; background: none; color: #2b5fb3; font-weight: 700; cursor: pointer;
