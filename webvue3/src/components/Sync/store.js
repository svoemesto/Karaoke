...
export default {
    ...
    getters: {
        getSyncEntities(state) { return state.entities },
        getSyncEntitiesIsLoading(state) { return state.entitiesIsLoading },
    },
    mutations: {
        setSyncEntities(state, entities) { state.entities = entities },
        setSyncEntitiesIsLoading(state, isLoading) { state.entitiesIsLoading = isLoading },
    },
    actions: {
        loadSyncEntitiesPromise(ctx) {
            ctx.commit('setSyncEntitiesIsLoading', true);
            return promisedXMLHttpRequest({method: 'GET', url: '/api/sync/entities'}).then(data => {
