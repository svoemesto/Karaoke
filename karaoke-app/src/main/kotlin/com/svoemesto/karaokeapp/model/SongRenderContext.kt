...
/**
 * Обёртка «песня + версия рендера» для MLT-генерации (не БД-сущность — см. [Song] для
...
//    private val REPLACE_STRING = "[R]"
    private val endString = "[E]"

//    private val COMMENT_STRING = "[C]"
    private val repeat1StartString = "[RS]"
    private val repeat1EndString = "[RE]"
    private val repeat1PastString = "[RR]"
    private val repeat2StartString = "[TS]"
    private val repeat2EndString = "[TE]"
    private val repeat2PastString = "[TR]"
    ...
    var capo: Int = 0
    var voices: MutableList<SongRenderVoice> = mutableListOf()
    val hasChords: Boolean get() = voices.sumOf { voice -> voice.lines.filter { it.type == SongRenderVoiceLineType.CHORDS }.size } > 0
...
//            val beatMs = if (song.ms == 0L) (60000.0 / song.bpm).toLong() else song.ms
            ...
            if (bodyText != "") {
                ...
                if (mapFiles.isNotEmpty()) {
                    ...
                    if (bodySubs.contains(endString)) {
                        ...
                        blocks.forEach { block ->
                            ...
                            if (startEnd != "" && id != 0L) {
                                ...
                                if (text.contains(repeat1StartString) || text.contains(repeat2StartString)) {
                                    // Устанавливаем флаги и пропускаем этот саб
                                    if (text.contains(repeat1StartString)) {
                                        isRepeated1 = true
                                        nextSubIsFirstRepeated1 = true
                                    }
                                    if (text.contains(repeat2StartString)) {
                                        isRepeated2 = true
                                        nextSubIsFirstRepeated2 = true
                                    }
                                } else {
                                    // Если текущий саб - отметка конца повтора
                                    if (text.contains(repeat1EndString) || text.contains(repeat2EndString)) {
                                        // Снимаем флаг и пропускаем этот саб
                                        if (text.contains(repeat1EndString)) {
                                            isRepeated1 = false
                                        }
                                        if (text.contains(repeat2EndString)) {
                                            isRepeated2 = false
                                        }
                                    } else {

                                        // Если текущий саб - отметка начала вставки повтора
                                        if (text.contains(repeat1PastString)) {
                                        ...
                                        } else if (text.contains(repeat2PastString)) {
                                            // Надо добавить в общий лист все элементы из повторного листа со сдвигом на оффсет
                                            val repeatPastOffset = startFrame
                                            ...
                                            listSubtitleFileElements.add(
                                                SubtitleFileElement(
                                                    ...
                                                    false,
                                                    (
                                                        text.uppercase().startsWith("[SETTING]|") ||
                                                            text == "//\\\\"
                                                    ),
                                                ),
                                            )
                                            ...
                                            if (isRepeated1) {
                                                listSubtitleFileElementsRepeated1.add(
                                                    SubtitleFileElement(
                                                        startFrame,
                                                        endFrame,
                                                        text,
                                            ...
                                            if (isRepeated2) {
                                                listSubtitleFileElementsRepeated2.add(
                                                    SubtitleFileElement(
                                                        startFrame,
                                                        endFrame,
                                                        text,
                        ...
                        for (i in 0 until listSubtitleFileElements.size - 1) {
                            ...
                            if (!currSubElement.isSetting && !currSubElement.text.contains(endString)) {
                                currSubElement.endFrame = nextSubElement.startFrame
                                currSubElement.isStartOfLine = isStartLine
                                currSubElement.isEndOfLine = nextSubElement.text.contains(endString)
                                isStartLine = currSubElement.isEndOfLine
                                if (indexSlog < slogs.size) {
                                    val txt =
                                        if (currSubElement.isEndOfLine) {
                                            slogs[indexSlog].substring(
                                                0,
                                                slogs[indexSlog].length - 1,
                                            )
                                        } else {
                                            slogs[indexSlog]
                                        }
                                    currSubElement.text =
                                        "${if (currSubElement.isStartOfLine) "//" else ""}${if (currSubElement.isStartOfLine) {
                                            txt
                                                .uppercaseFirstLetter()
                                        } else {
                                            txt
                                        }}${if (currSubElement.isEndOfLine) "\\\\" else ""}"
                                    indexSlog++
                                    if (indexSlog == slogs.size) {
                        ...
                        var newBody = ""
                        listSubtitleFileElements.filter { !it.text.contains(endString) }.forEachIndexed { index, element ->
                            newBody +=
                                "${index + 1}\n${convertFramesToTimecode(
                                    element.startFrame,
                                ).replace(".",",")} --> ${convertFramesToTimecode(element.endFrame).replace(".",",")}\n${element.text}\n\n"
                        }
            ...
            val listVoices = mutableListOf<SongRenderVoice>()
            for (voiceId in listFile.indices) {

                val chords: MutableList<Chord> = mutableListOf()
                val listLines = mutableListOf<SongRenderVoiceLine>()
                val listChordLines = mutableListOf<SongRenderVoiceLine>()
                var maxLengthLine = 0
                ...
                val body = File(mapFiles[voiceId]).readText(Charsets.UTF_8)
                        ...
                        if (text.uppercase().startsWith("[SETTING]|")) {
                            ...
                            when (settingList[1].uppercase()) {
                                ...
                                "COMMENT" -> {
                                    ...
                                    listLines.add(
                                        SongRenderVoiceLine(
                                            type = SongRenderVoiceLineType.COMMENTS,
...
//                            mltText = Karaoke.voices[0].groups[group].mltText.copy(text),
                            ...
                            if (isLineEnd) {
                                ...
                                if (songVersion != SongVersion.CHORDS && lineText.containOnlyThisSymbols(NOTES_SYMBOLS)) {
                                    listLines.add(
                                        SongRenderVoiceLine(
                                            type = SongRenderVoiceLineType.EMPTY,
                                            ...
                                            durationMs =
                                                getDurationInMilliseconds(
                                                    songSubtitles.first().startTimecode,
                                                    songSubtitles.last().endTimecode,
                                                ),
                                    ...
                                    listLines.add(
                                        SongRenderVoiceLine(
                                            type = if (lineText.trim() == "") SongRenderVoiceLineType.EMPTY else SongRenderVoiceLineType.TEXT,
                ...
                listLines.forEach { voiceLine ->
                    val symbolsChords = mutableListOf<SongRenderVoiceLineSymbol>()
                    val subtitlesChords = mutableListOf<Subtitle>()
                    chords.forEach { chord ->
                        // Находим субтитр текущей строки, на который попадает аккорд
                        val subtitle =
                            voiceLine.subtitles.firstOrNull {
                                convertTimecodeToMilliseconds(chord.timecode) in
                                    convertTimecodeToMilliseconds(it.startTimecode) until convertTimecodeToMilliseconds(it.endTimecode)
                            }
                    ...
                    if (symbolsChords.isNotEmpty()) {
                        ...
                        listChordLines.add(
                            SongRenderVoiceLine(
                                type = SongRenderVoiceLineType.CHORDS,
...
//        endTimecode = voices.first().lines.last().subtitles.last().endTimecode
            if (listVoices.isNotEmpty() && listVoices.first().lines.isNotEmpty()) {
                endTimecode =
                    voices
                        .first()
                        .lines
    ...
    fun setMaxLines() {
        voices.forEachIndexed { indexVoice, currVoice ->
            val currVoiceTextLines = currVoice.lines.filter { it.type == SongRenderVoiceLineType.TEXT }
...
//            val maxSingleLineWidthPx = 0
            ...
            if (currVoice != voices.last()) {
                ...
                currVoiceTextLines.forEach { currVoiceTextLine ->
                    ...
                    val nextVoiceLines = mutableListOf<SongRenderVoiceLine>()
                    ...
                    val nextVoiceTextLines = nextVoice.lines.filter { it.type == SongRenderVoiceLineType.TEXT }
...
data class SongRenderVoice(
    ...
    var lines: MutableList<SongRenderVoiceLine> = mutableListOf(),
) : Serializable {
    @Suppress("unused")
    ...
    val maxCountSymbolsInLine: Int
        get() {
            return lines
                .filter { it.type == SongRenderVoiceLineType.TEXT }
                .firstOrNull { it.isMaxLine }
                ?.text
                ?.length ?: 0
        }
    ...
    val maxWidthLinePx: Long
        get() {
            return lines.filter { it.type == SongRenderVoiceLineType.TEXT }.firstOrNull { it.isMaxLine }?.widthLinePx ?: 0
        }

    @Suppress("unused")
    val maxWidthSingleLinePx: Long
        get() {
            return lines.filter { it.type == SongRenderVoiceLineType.TEXT }.firstOrNull { it.isMaxSingleLine }?.widthLinePx ?: 0
        }

    @Suppress("unused")
    val maxWidthLineText: String
        get() {
            return lines.filter { it.type == SongRenderVoiceLineType.TEXT }.firstOrNull { it.isMaxLine }?.text ?: ""
        }

    @Suppress("unused")
    val maxWidthSingleLineText: String
        get() {
            return lines.filter { it.type == SongRenderVoiceLineType.TEXT }.firstOrNull { it.isMaxSingleLine }?.text ?: ""
        }

    fun getScreenY(
        line: SongRenderVoiceLine,
        time: String,
        horizonPositionPx: Int,
    ): Int {
        var result = 0
        if (line in lines) {
//            val indexLine = lines.indexOf(line)
            ...
            val lineContainTime =
                lines
                    .filter { it.startTp != null && it.endTp != null && it.type == SongRenderVoiceLineType.TEXT }
                    .firstOrNull {
                        convertTimecodeToMilliseconds(it.startTp!!.time) <= timeMs &&
                            convertTimecodeToMilliseconds(it.endTp!!.time) >= timeMs
                    }
            if (lineContainTime != null) {
                ...
                if (lineBeforeTime == null) {
                    ...
                    val lineInTime = lines.first { it.startTp != null && it.endTp != null && it.type == SongRenderVoiceLineType.TEXT }
                    ...
                    val lineInTime = lines.last { it.startTp != null && it.endTp != null && it.type == SongRenderVoiceLineType.TEXT }
                    result = horizonPositionPx - line.hPx - (line.yPx - lineInTime.yPx).absoluteValue
                } else {
...
data class SongRenderVoiceLine(
    ...
    var symbols: MutableList<SongRenderVoiceLineSymbol> = mutableListOf(),
    var group: Int = 0,
    var yPx: Int = 0,
    var text: String = "",
    var start: String = "",
    ...
    @Suppress("unused")
    fun getFillTps(
        voice: SongRenderVoice,
        horizonPositionPx: Int,
        opacityFillValue: Double,
    ): String {
        ...
        val nextLine =
            voice.lines.firstOrNull {
                it.type == SongRenderVoiceLineType.TEXT &&
                    it.startTp != null &&
                    convertTimecodeToMilliseconds(it.startTp!!.time) > endTpCurrLineMs
            }
...
//        val nextNextLine : SongRenderVoiceLine? = if (nextLine == null) {
//            null
//        } else {
//            voice.lines.firstOrNull {
//                it.type == SongRenderVoiceLineType.TEXT && it.startTp != null && convertTimecodeToMilliseconds(it.startTp!!.time) > convertTimecodeToMilliseconds(nextLine.endTp!!.time)
//            }
//        }
...
//        val timeToTimecodeNextNext = convertMillisecondsToTimecode(timeToMsNextNext)
        ...
        var yEnd: Int
        // Проходимся по каждому субтитру строки
...
//                println("${subtitle.mltText.text} - ${timeToMs} - ${endTpCurrLineMs} - ${yEnd}")
        ...
        if (subtitles.isNotEmpty()) {
            ...
            endTimeFadeMs =
                if (endTpNextLineMs > 0 && endTpNextLineMs < endTimeFadeMs) {
            ...
            if (startTpNextLineMs >= endTimeFadeMs) {
                ...
                val coeff =
                    (endTimeFadeMs.toDouble() - startTpNextLineMs.toDouble()) / (endTimeFadeMs.toDouble() - endTpCurrLineMs.toDouble())
                // До начала анимации следующей линии фейдим до коэффициента
                if (nextLine != null) {
                    // Находим положение текущей линии на момент начала следующей линии
                    yEnd =
                        voice.getScreenY(this, convertMillisecondsToTimecode(startTpNextLineMs), horizonPositionPx) +
                        subtitles.last().deltaEndY
                    result.add(
                        TransformProperty(
    ...
    @Suppress("unused")
    fun getSubtitleWpx(subtitle: Subtitle): Double = getTextWidthHeightPx(subtitle.mltText.text, mltText.font).first
