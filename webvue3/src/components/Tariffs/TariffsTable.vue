<template>
  <div class="trf-wrapper">
    ...
    <div class="trf-add">
      ...
      <input
        v-model.number="newItem.priceRub"
        class="trf-input trf-num"
        type="number"
        min="0"
        step="0.01"
        placeholder="Цена, ₽"
      />
      <input
        v-if="newItem.scope === 'SITE'"
        v-model.number="newItem.periodDays"
        class="trf-input trf-num"
        type="number"
        min="1"
    ...
    <table v-else class="trf-table">
      ...
      <tbody>
        <tr v-for="item in editable" :key="item.id">
          ...
          <td>
            <input
              v-model.number="item.periodDays"
              class="trf-input trf-num"
              type="number"
              min="0"
              :disabled="item.scope === 'SONG'"
            />
          </td>
          ...
          <td class="trf-actions">
            <button class="trf-btn" :disabled="!isChanged(item)" @click="save(item)">
              Сохранить
            </button>
...
export default {
  ...
  computed: {
    isLoading() {
      return this.$store.getters.getTariffsIsLoading
    },
    list() {
      return this.$store.getters.getTariffsList
    },
    target: {
      get() {
        return this.$store.getters.getTariffsTarget
      },
      set(v) {
        this.$store.dispatch('setTariffsTarget', v)
      },
    },
    canCreate() {
      return this.newItem.name.trim() !== '' && this.newItem.priceRub >= 0
    },
  },
  watch: {
    list: {
