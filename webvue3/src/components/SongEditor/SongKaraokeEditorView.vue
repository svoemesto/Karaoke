    ...
    <div v-if="canEdit && showKeyboard" class="ske-keyboard">
      <div class="ske-kb-grid">
        <div v-for="(row, ri) in keyboardRows" :key="ri" class="ske-kb-row">
          <button
            v-for="(k, ki) in row.keys"
            :key="k.code || 'blank-' + ri + '-' + ki"
            type="button"
            class="ske-kb-key"
            :style="{ flexBasis: k.w * 56 + (k.w - 1) * 6 + 'px' }"
            :class="{
              'ske-kb-key-active': k.code && heldKeys[k.code],
              'ske-kb-key-inactive': !k.code,
              'ske-kb-key-spacer': k.spacer,
            }"
            @mousedown.prevent="pressKey(k.code)"
            @mouseup.prevent="releaseKey(k.code)"
    ...
    <div class="ske-texts">
      <div class="ske-text-col">
        ...
        <textarea
          ...
          :placeholder="
            canEdit ? 'Вставьте сюда текст песни — он автоматически разобьётся на слоги.' : ''
          "
          @input="onTextInput"
        />
      </div>
...
export default {
  ...
  computed: {
    ...
    statusKind() {
      return this.status || 'song'
    },
    showRejectBanner() {
      return this.mode === 'assignment' && this.status === 'rejected' && this.reviewComment
    },
    currentVoiceData() {
      return this.voices[this.currentVoiceIdx] || { sourceText: '', markers: [], syllables: [] }
    },
    sourceText: {
      get() {
        return this.currentVoiceData.sourceText
      },
      set(v) {
        if (this.voices[this.currentVoiceIdx]) this.voices[this.currentVoiceIdx].sourceText = v
      },
    },
    markers: {
      get() {
        return this.currentVoiceData.markers
      },
      set(v) {
        if (this.voices[this.currentVoiceIdx]) this.voices[this.currentVoiceIdx].markers = v
      },
    },
    syllables: {
      get() {
        return this.currentVoiceData.syllables
      },
      set(v) {
        if (this.voices[this.currentVoiceIdx]) this.voices[this.currentVoiceIdx].syllables = v
      },
    },
    hasAnyMarkers() {
      return this.voices.some((v) => v.markers.length > 0)
    },
    curSyllableIndex() {
      return currentSyllableIndex(this.markers, this.currentTime)
    },
    curMarkerIndex() {
      return currentMarkerIndex(this.markers, this.currentTime)
    },
    formattedTextHtml() {
      return formatText(this.markers, this.curMarkerIndex)
    },
    tail() {
      return buildTail(this.syllables, this.curSyllableIndex)
    },
    keyboardRows() {
  ...
  watch: {
    ...
    volume(v) {
      saveEditorSettings({ volume: v })
    },
    playbackRate(v) {
      saveEditorSettings({ playbackRate: v })
    },
    zoom(v) {
      saveEditorSettings({ zoom: v })
    },
    activeSound(v) {
      saveEditorSettings({ activeSound: v })
    },
    showKeyboard(v) {
      saveEditorSettings({ showKeyboard: v })
    },
  },
  async mounted() {
  ...
  beforeUnmount() {
    ...
    try {
      this.kpPlayer && this.kpPlayer.destroy()
    } catch (e) {
      /* noop */
    }
  },
  methods: {
    ...
    loadVoicesFromProps() {
      ...
      const rawMarkers =
        this.markersPerVoice && this.markersPerVoice.length ? this.markersPerVoice : [[]]
    ...
    step(sec) {
      if (this.ws)
        this.ws.setTime(Math.max(0, Math.min(this.duration, this.ws.getCurrentTime() + sec)))
    },
    applyRate() {
    ...
    applyVolume() {
      if (this.ws) this.ws.setVolume(this.volume)
    },
    async setActiveSound(sound) {
      if (this.activeSound === sound || !this.ws) {
    ...
    removeLastVoice() {
      ...
      if (
        !window.confirm(
          `Удалить голос ${this.voices.length}? Весь его текст и маркеры будут потеряны.`,
        )
      )
        return
      this.voices.pop()
    ...
    clearMarkers() {
      if (!this.canEdit) return
      if (!window.confirm('Удалить все маркеры разметки? Отменить это действие будет нельзя.'))
        return
      this.markers = []
      this.redrawRegions()
      this.$emit('change')
    ...
    async togglePlayer() {
      ...
      try {
        ...
        this.kpPlayer = new KaraokePlayer(
          this.$refs.playerContainer,
          { inlineData, containerHeight: initialH + 'px' },
          '/api',
        )
    ...
    computePlayerHeight16x9(wrap) {
      ...
      if (w <= 0) return 0
      const canvasWrap =
        this.$refs.playerContainer && this.$refs.playerContainer.querySelector('#kp-canvas-wrap')
      const rootDiv = canvasWrap ? canvasWrap.parentElement : null
      let controlsH = 110
    ...
    dispatchKeyAction(code) {
      switch (code) {
        ...
        case 'KeyX':
          ...
          break
        case 'KeyQ':
          if (this.ws) this.ws.skip(-0.01)
          break
        case 'KeyE':
          if (this.ws) this.ws.skip(0.01)
          break
        case 'KeyA':
          this.startScrub('KeyA', -1, false)
          break
        case 'KeyD':
          this.startScrub('KeyD', 1, false)
          break
        case 'KeyZ':
          this.startScrub('KeyZ', -1, true)
          break
        case 'KeyC':
          this.startScrub('KeyC', 1, true)
          break
        case 'BracketLeft':
          this.jumpMarker(-1, null)
          this.startNavRepeat('BracketLeft', -1, null)
          break
        case 'BracketRight':
          this.jumpMarker(1, null)
          this.startNavRepeat('BracketRight', 1, null)
          break
        case 'Comma':
          this.jumpMarker(-1, 'syllables')
          this.startNavRepeat('Comma', -1, 'syllables')
          break
        case 'Period':
          this.jumpMarker(1, 'syllables')
          this.startNavRepeat('Period', 1, 'syllables')
          break
        case 'KeyS':
          if (this.canEdit) this.removeMarker()
          break
        case 'KeyW':
          if (this.canEdit) this.mark('syllables')
          break
        case 'Digit1':
          if (this.canEdit) this.mark('endofsyllable')
          break
        case 'Digit2':
          if (this.canEdit) this.mark('endofline')
          break
        case 'Digit3':
          if (this.canEdit) {
            this.mark('endofline')
            this.mark('syllables', true)
          }
          break
        case 'Digit4':
          if (this.canEdit) this.mark('newline')
          break
        case 'Digit5':
          if (this.canEdit) {
            this.mark('endofline')
            this.mark('newline', true)
            this.mark('syllables', true)
          }
          break
        case 'KeyT':
          if (this.canEdit) this.mark('setting', false, 'GROUP|0')
          break
        case 'KeyY':
          if (this.canEdit) this.mark('setting', false, 'GROUP|1')
          break
        case 'KeyU':
          if (this.canEdit) this.mark('setting', false, 'GROUP|2')
          break
        case 'KeyI':
          if (this.canEdit) this.mark('setting', false, 'GROUP|3')
          break
        case 'KeyP':
          if (this.canEdit) this.mark('setting', false, 'GROUP|4')
          break
        case 'KeyO':
          this.addComment()
          break
    ...
    startScrub(code, direction, fast) {
      ...
      if (fast) {
        ...
        this.ws.setPlaybackRate(this.editSpeed)
        this.scrubTimers[code] = setInterval(() => {
          if (!this.ws.isPlaying()) this.ws.play()
        }, 16)
      }
    },
    ...
    stopScrub(code) {
      if (this.scrubTimers[code]) {
        clearInterval(this.scrubTimers[code])
        delete this.scrubTimers[code]
      }
...
.ske-voice-tab-add:hover {
  background: rgba(36, 128, 58, 0.1);
}
.ske-voice-tab-remove {
  border-color: #c0392b;
  color: #c0392b;
  background: transparent;
}
.ske-voice-tab-remove:hover {
...
.ske-kb-key-active .ske-kb-key-label,
.ske-kb-key-active .ske-kb-key-caption {
  color: #fff;
}
.ske-kb-key-inactive {
  opacity: 0.32;
...
.ske-kb-key-inactive:hover {
  background: #ffffff;
}
.ske-kb-key-spacer {
  padding: 0.2rem 0.05rem;
}
.ske-kb-key-spacer .ske-kb-key-label {
  font-size: 0.8rem;
}
...
.ske-preview :deep(.ske-fx-cur) {
  color: #ff0000;
  font-weight: 700;
}
.ske-preview :deep(.ske-fx-group0) {
  color: #ffffff;
}
.ske-preview :deep(.ske-fx-group1) {
  color: #ffff00;
  font-style: italic;
}
.ske-preview :deep(.ske-fx-group2) {
  color: #00bfff;
}
.ske-preview :deep(.ske-fx-group3) {
  color: #00ff00;
  font-style: italic;
}
.ske-preview :deep(.ske-fx-comment) {
  color: #d2691e;
  font-size: 0.78em;
  font-style: italic;
}
