...
| 10 | `docs/strategy/growth.md` | **Стратегия роста** (воронка, roadmap, гипотезы) | ~150 строк |

**При расхождении** — приоритет у файла с меньшим номером.
**AI-агент**: читай файлы в этом порядке при старте сессии (1 → 9, стратегию — по запросу про рост/воронку).
...
## Стратегия роста (воронка visitor→registration→premium)

> **Cross-ref**: полная стратегия — [`docs/strategy/growth.md`](./docs/strategy/growth.md),
> аудит со всеми гипотезами — [`docs/strategy/growth-audit.md`](./docs/strategy/growth-audit.md).

**Baseline** (≈ месяц 2026-07): 5 284 уникальных / мес, 20 регистраций, 3 премиума.
**Главная проблема**: 0.4% visitor→registration (норма 2-5%, у нас в 5-12× ниже).
**Главный фокус**: visitor→registration. Модель монетизации (75% free / 25% premium) — вторичный приоритет.

**Принятые решения:**
- **Модель D (гибрид)**: эфир-N-дней + 1 трек/исполнитель + 1 альбом/исполнитель free. Реализация — после M-15 (анализ `tbl_settings.album`).
- **11 097 grandfathered**: «решим позже», в первом раунде не трогаем.
- **Без trial** в первом раунде: anti-fraud дороже выгоды. QW-3, M-11, T1.x-T3.x — в backlog.
- **Топ-3 фичи первого раунда** (spec.md готовятся):
  - **QW-9** — Страница «О проекте» (русский рок, исполнители, каталог)
  - **QW-2** — «5 причин зарегистрироваться» на главной
  - **QW-1** — Таблица «FREE vs PREMIUM» на /premium

**Roadmap** (12 QW + 22 M + 11 L) — в [`docs/strategy/growth.md#6-roadmap`](./docs/strategy/growth.md#6-roadmap).
...
**Что НЕ делаем в первом раунде** (backlog):
- Trial (QW-3, M-11)
- Anti-trial-abuse (T1.x-T3.x)
- «Подарочный» / «корпоративный» тарифы
...
- **На хосте `dev-pc` под пользователем `dev`**: пересобирать/перезапускать **любой** локальный
  контейнер проекта, включая `karaoke-app`, без дополнительного согласия пользователя
- **На хосте `dev-pc` под пользователем `dev`**: выполнять любые операции с локальной базой
  данных (запросы, миграции, изменения схемы/данных) без дополнительного согласия пользователя.
  Это не распространяется на серверную (прод) БД — см. пункт «Деплоить на сервер» выше
...
5. Только после зелёного CI: `gh pr merge --merge`. **Ветку НЕ удалять** —
   см. ниже «Жизненный цикл feature-ветки (NON-NEGOTIABLE)».
...
**без** `--delete-branch`. После успешного merge в master ветка
`NNN-slug` остаётся жить локально и на origin, чтобы в неё можно было
докатывать мелкие правки той же фичи (review fixes, дополнительные
коммиты, новые сценарии quickstart).

**Почему.** Если нужно поправить замечание ревьюера или добавить
недостающий сценарий после мёрджа — это та же фича, а не новая. Создавать
ещё одну ветку и ещё один PR ради 2-строчной правки — лишний шум в
`git log` и `gh pr list`, плодит «фантомные» ветки в `seq/NNN`-тегах и
ломает естественную группировку истории фичи. Удаление ветки после
мёрджа — лишний шаг, который ничего не даёт: `gh` и так подсвечивает
смерженные PR как «Merged», а ветка остаётся в истории.

**Что делать вместо `--delete-branch`:**

```bash
# Мерджим без удаления:
gh pr merge <N> --merge

# Если нужно докатить правку — НЕ создаём новую ветку, а работаем
# в той же самой (даже если она уже смержена в master):
git checkout NNN-slug
# ... правки ...
git commit --amend                  # если правка логически часть
                                     # последнего коммита
# или
git commit -m "fix: ..."            # если это отдельный логический шаг
git push origin NNN-slug
gh pr create --base master --title "fix: ..." --body "..."
```

Если правка **меняет поведение фичи** — это нормальный follow-up PR
из той же ветки (ветка живёт, история накапливается). Если правка
**тривиальная** (опечатка, забытый JSDoc-тег) — допустимо сделать
её прямо в смерженной ветке через `git revert` + `git commit -m "fix: ..."`
+ новый PR; **не** через создание ветки `NNN-fix-...`.

**Что НЕ делать:**
- `gh pr merge --merge --delete-branch` (нарушение правила).
- `git branch -d NNN-slug` после мёрджа «за ненадобностью».
- Создавать новую ветку `NNN-fix-...` ради тривиальной правки.

**Локальный ref на смерженную ветку:** GitHub после мёрджа без
`--delete-branch` всё равно может удалить remote ref (зависит от
настроек репозитория). Если `git fetch --prune` убрал
`origin/NNN-slug` — это нормально, локальная ветка `NNN-slug` остаётся
жить у вас в `git branch`. Чтобы продолжить работу в ней:

```bash
git checkout NNN-slug                  # локальная ветка никуда не делась
git commit --amend                    # правка
git push origin NNN-slug              # воссоздаём remote ref
# Если GitHub отвергает push «branch was deleted» — force-push с lease:
# git push --force-with-lease origin NNN-slug
gh pr create --base master            # новый PR из той же ветки
```

**Исключение:** если ветка больше не нужна (фича отменена, откат
через `git revert` в master) — допустимо удалить её отдельным коммитом
с обоснованием в `docs/architecture-notes.md`. Это явно решается
по запросу пользователя, не автоматически.

### Нумерация feature-веток (NNN-slug)

Ветки именуются `NNN-slug` (`NNN` — трёхзначный порядковый номер с ведущими
нулями, `slug` — краткое описание на английском, kebab-case). Номер
**обязательно резервировать** перед созданием ветки:

```bash
# Зарезервировать номер и сразу создать+переключиться на ветку:
./tools/reserve-branch-number.sh my-feature-slug

# Или отдельно (например, если ветка создаётся другим инструментом):
N=$(./tools/reserve-branch-number.sh)
git checkout -b "${N}-my-feature-slug"
```

**Зачем.** При параллельной работе нескольких агентов/машин двое могут
одновременно посчитать «следующий номер» одинаковым — конфликт не на
git-уровне (имена веток разные, `git push` не падает), а по смыслу конвенции
(инцидент 2026-07-29: `029-fix-queue-lane-stall` и
`029-player-overlay-remove-share-move-fav` получили один номер в одной
сессии). Скрипт резервирует номер атомарно: пушит в origin lightweight-тег
`refs/tags/seq/NNN`. Git отклоняет push, если тег с таким именем уже
существует на origin — скрипт автоматически пробует следующий номер, пока не
найдёт свободный (`git`-native compare-and-swap, без отдельной
инфраструктуры).

**Не создавать ветку с номером вручную** (`git checkout -b 030-...` в обход
скрипта) — это именно то, что ломает конвенцию при параллельной работе.

**`/speckit-specify` — та же проблема, отдельная нумерация.** Skill
(`.claude/skills/speckit-specify/SKILL.md`, ванильный из `github-spec-kit`,
не патчить напрямую — слетит при обновлении фреймворка) сам считает `NNN`
для `specs/NNN-slug`, сканируя **только** локальную папку `specs/` — про
git-ветки и тем более про другие машины он не знает вообще. Отсюда та же
гонка, что и с ветками (инцидент 2026-07-29: специфика после `029-...`
независимо получила `030` на другой машине, столкнувшись с уже
зарезервированным через скрипт номером ветки). Fix — не патч skill'а, а
использование его же документированной точки расширения: skill обязан
уважать явно переданный `SPECIFY_FEATURE_DIRECTORY` (см. SKILL.md,
"Resolution order for SPECIFY_FEATURE_DIRECTORY", пункт 1). Поэтому:

```bash
# Перед /speckit-specify — зарезервировать номер (не создавая ветку, слага может
# ещё не быть/отличаться) и явно передать директорию:
N=$(./tools/reserve-branch-number.sh)
export SPECIFY_FEATURE_DIRECTORY="specs/${N}-my-feature-slug"
# Затем вызвать /speckit-specify как обычно — skill возьмёт этот путь как есть,
# а не будет пересчитывать NNN сканированием specs/.
```

Ветку для той же фичи создавать тем же `N` (`git checkout -b "${N}-my-feature-slug"`),
чтобы номер ветки и номер спеки совпадали, как исторически сложилось в проекте
(SKILL.md прямо говорит, что это не обязательно фреймворком — но здесь это
конвенция проекта).
...
Visitor→registration (конверсия 0.4%, норма 2-5%, потенциал ×5-13). Модель монетизации —
вторичный приоритет. Подробнее — [`docs/strategy/growth.md`](./docs/strategy/growth.md).
При планировании фич по воронке — сначала visitor→registration (QW-9, QW-2, QW-1).

### Q: Делаем ли мы trial (бесплатный премиум на N дней) в первом раунде?

**Нет.** Anti-fraud (T1.x-T3.x) дороже выгоды при текущей базе. QW-3, M-11 — в backlog.
Если/когда вернёмся к trial — обязательно сначала M-11 (anti-trial-abuse уровень 1:
email-cooldown + fingerprint + IP rate limit + disposable email block).

### Q: Что с 11 097 уже эфирных песен?

**Grandfathered**: «решим позже», в первом раунде не трогаем. Если позже решим переводить
в premium — нужен A/B-тест и метрика популярности.
