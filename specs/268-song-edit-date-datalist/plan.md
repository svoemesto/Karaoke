...
|-----------|------------|-------------------------------------|
| Не создан новый per-feature документ (`docs/features/268-*.md`) в реестре `docs/features/README.md`, хотя FR-009 Конституции упоминает per-feature документ при правке кода ключевой подсистемы | Фича — точечный 4-атрибутный frontend-фикс, не отдельная продуктовая подсистема. Создание нового документа ради 4 атрибутов добавляет обслуживающий груз (README.md таблица реестра, отдельный `.md`-файл) непропорционально размеру изменения. Существующий LiveDoc `156-publish-slots-range.md` уже покрывает эту функциональность (UI-форма + backend), и фронт-фикс органично туда вписывается как запись в «История» | 1) Создать `docs/features/268-song-edit-date-datalist.md` — избыточно для одно-атрибутного фикса; пользователь может завести документ позже, если потребуется. 2) Расширить `songs-table.md` — не подходит: тот документ про таблицу списка песен, а не про форму редактирования одной песни, это разные UI-контексты |