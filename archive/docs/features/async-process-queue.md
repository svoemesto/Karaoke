...
- **Санитайзинг имени файла при импорте, падение Demucs на `!`** (устранено в
  specs/124-filename-sanitization-rename, 2026-08-03): `Song.createFromPath()`
  никогда не санитайзировал `fileName` перед перекодированием в FLAC — файл
  на диске оставался с оригинальными символами (например `!`), а более
  поздние шаги пайплайна (копирование в temp-папку Demucs) применяли
  санитайзинг только к целевому пути, не к реальному имени файла на диске —
  `cp` не находил исходник, и разделение на стемы падало. Исправлено:
  `fileName` санитайзируется через `String.sanitizeSongFileName()`
  (`Extentions.kt`) сразу после regex-парсинга, ДО перекодирования/копирования
  физического файла; `SongField.NAME` (отображаемое название) сохраняет
  оригинальные символы. Коллизия санитайзированного имени между двумя РАЗНЫМИ
  файлами одного вызова `createFromPath()` разрешается автоматическим
  числовым суффиксом (`" (2)"`, `" (3)"`, ...), не затрагивая проверку
  "уже импортировано" для повторных запусков импорта над той же папкой.
- **Каскадное переименование файла песни через SongEdit** (реализовано в
  specs/124-filename-sanitization-rename, 2026-08-03): при сохранении
  изменённого «Имя файла» в SongEdit (`ApiController.songs2Update`) значение
  теперь санитайзируется тем же `String.sanitizeSongFileName()`, отклоняется
  при коллизии с другой песней той же папки, пустом результате санитайзинга
  или наличии активного `KaraokeProcess` для этой песни (`WAITING`/`WORKING`,
  новый хелпер `KaraokeProcess.hasActiveProcess()`) — ответ эндпоинта
  расширен до `SongUpdateResultDto(albumLinkValid, fileNameRenameError)`.
  Обнаружено, что каскадное переименование ОСНОВНОГО аудио/FLAC/WAV-вариантов
  стемов уже существовало (`Song.renameFilesIfDiff()`, вызывается автоматически
  из `saveToDb()` при любом изменении `fileName` — не специфично для SongEdit)
  — этой фичей добавлено недостающее: `.kdenlive`-проект и файл субтитров,
  устаревший sidecar-файл метаданных под старым именем, mp3-варианты стемов на
  диске и в локальном/удалённом хранилище (`Song.renameCascadeExtraArtifacts()`).
  Best-effort вперёд: ошибка на одном артефакте не откатывает уже
  переименованное; частичный отказ проявляется через обычный пересчёт
