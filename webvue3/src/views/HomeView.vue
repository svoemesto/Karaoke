    ...
    customFunction() {
      this.customConfirmParams = {
        ...
        body:
          ...
          `Для каждой такой песни сначала будет выполнен поиск родителя по точному совпадению названия (текст/маркеры переписываются, кроме случая, когда у песни уже есть проверенный текст — статус ≥ TEXT_CHECK), затем — поиск аудио-родителя по акустическому сходству (порог 85%, поле не пересекается с родителем).<br>` +
