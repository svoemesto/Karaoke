export default class KaraokePlayer {
  ...
  async init() {
    ...
    try {
      ...
      if (loading) { loading.textContent = `Ошибка: ${e.message}`; loading.style.background = 'transparent' }
    }
  }
  ...
  _buildUI() {
    ...
    this.container.innerHTML = `
      <div style="display:flex;flex-direction:column;height:100vh;background:#000">
        ...
        <div style="background:#111;border-top:1px solid #333;padding:6px 12px;display:flex;align-items:center;gap:10px">
          ...
          <button id="kp-load" title="Загрузить .smkaraoke" style="background:none;border:none;color:#ccc;font-size:16px;cursor:pointer;padding:0 4px">📂</button>
        </div>
      </div>`
    ...
    this.container.querySelector('#kp-load').addEventListener('click', () => {
  ...
  _renderFrame() {
    ...
    if (!this._ready) { this._updateControls(dt); return }
  ...
  _renderSplash(ctx, W, H, _scale, ct, splashDur) {
    ...
    ctx.font = `400 ${chordSz}px Roboto, sans-serif`
