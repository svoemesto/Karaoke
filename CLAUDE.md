...
утилитарные). Live-обновления — SockJS/STOMP (`assets/js/sockjs.js`, `stomp.js`). Auth — OAuth2/OIDC против
...
- **Онлайн-редактор разметки (`/account/editor`) с модерацией.** Упрощённый публичный аналог `SubsEdit.vue`:
  ...
  Детали → memory `project_song_editor`.
...
долетает до psql). Любая правка схемы `tbl_events` обязана попасть в `deploy/recordhash_events.sql` (иначе
LOCAL↔SERVER sync перестаёт видеть diff по новым колонкам).
