<template>
  <div class="at-grid at-modern">
    ...
    <!-- Слот для доп. тайлов, которые должны попасть в ТУ ЖЕ сетку, что и обычные авторы
         (например виртуальная плашка «Отдельные песни разных авторов» в ZakromaView.vue) —
         .at-grid растягивает/выравнивает любой прямой потомок одинаково, вне зависимости
         от того, кто его отрендерил. -->
    <slot name="trailing" />
  </div>
</template>
 ...
 *
 * @emits select
 * @slot trailing - доп. тайл(ы) в конце той же сетки (например спец-плашка «Отдельные
 *   песни разных авторов» в ZakromaView.vue)
 *
 * @see AGENTS.md
 */
export default {
