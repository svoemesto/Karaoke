---
status: Archived
slug: architecture-notes-archive
type: changelog
related:
  - ./architecture-notes.md
---

# Architecture notes — архив (детальная история фич и «ловушек»)

> **Что это.** [удалено] реализованных фич и
> отловленных багов/«ловушек» (в основном датированные записи <DATE>…07). Он **не загружается
> автоматически** в контекст [AI-лог] Code (в отличие от `[AI-лог].md`) — открывать вручную (Read/grep),
> когда нужны детали конкретной фичи или ловушки.
>
> **Зачем вынесено.** [удалено]. Durable-
> карта архитектуры, команды сборки/деплоя и ключевые инварианты остались в `[AI-лог].md`; сюда
> переехала историческая детализация.
>
> **Дублирование.** Практически каждый блок ниже также сжато зафиксирован отдельным memory-файлом в
> `[удалено]` (индекс — [удалено]). Memory подтягивается on-demand
> по описанию; этот архив — полный текст-первоисточник и бэкап. Git-история (`git log`/`blame` по
> `[AI-лог].md`) хранит те же записи как коммиты.
>
> **Актуальность.** Записи отражают состояние на момент написания. Если запись называет файл/функцию/
> флаг — проверяй, что он ещё существует, прежде чем опираться.

---

## Часть 1. Architecture notes (ядро: БД, sync, пайплайн процессов, storage, HealthReport, статистика)
...
- **`rowChunkSize`** (per-table в `SyncTarget`) — для полнострочных READ (`loadByIds`, `SELECT * WHERE id IN`),
 ...
 `siteusers` 500, `events` 100 (снижен с 500 <DATE>: `SELECT * WHERE id IN (500)` по remote-
...
**Диагностика «бесконечной» реконсиляции events — не всегда рассинхрон функции хэша.**
...
из репозитория, это два разных файла, реально используемый лежит вне репо в `/<RUNTIME_PATH>/deploy`).
 ...
 **не поддерживает флаг `--cpus` вообще**, падает `unknown flag: --cpus` при любом запуске, если
 ...
 - **`/<RUNTIME_PATH>/mlt-docker/docker-compose.yaml` — системный файл ВНЕ git-репозитория** (путь
 ...
 **Ловушка с версией cpulimit (наступали <DATE>).** Установлен `cpulimit` **2.4** (пакет `2.7-2`):
...
(`/<RUNTIME_PATH>/Karaoke.properties`) and editable through the Properties UI/API rather than recompiling.
...
Функция возвращает `AlbumSearchResult` (sealed class): `Success`, `проксиBlocked`, `AuthExpired`, `BotDetected`, `Unknown`.

**Предварительная Проверка доступности (`isпроксиActive()` в `Utils.kt`).**
Перед вызовом `checkLastAlbumYm()` в `KaraokeProcessWorker` вызывается `isпроксиActive()` — лёгкий HTTP-запрос
к `api.country.is` (запасной: `ipapi.co/country/`). Если страна не `RU` — Playwright не запускается,
в лог выводится `"Проверка нового альбома пропущена — прокси включён"`. При исключении (сервис недоступен)
...
- **прокси**: HTML содержит `"недоступна в вашем регионе"` → сообщение "отключите прокси".
- **Авторизация**: `page.url()` после навигации содержит `passport.yandex` или `id.yandex` → сообщение "Переавторизуйтесь".
- **Unknown (резерв)**: запрос к `api.country.is`; если страна не `RU` — тоже прокси с кодом региона; иначе — сообщение об изменении кода страницы с `page.title()` и URL для диагностики.
HTML страницы в лог не выводится.

Коды `reason` из `checkLastAlbumYm` (обрабатываются в `KaraokeProcessWorker`):
- `-3` — прокси или авторизация: таймаут **не** увеличивается, `requestNewSongLastSuccessAuthor` **не** меняется (следующая попытка — тот же автор, тот же интервал).
...
**MinIO на отдельном сервере (<MINIO_IP>) — MTU-проблема и обязательный паттерн для karaoke-web.**
Docker-контейнер karaoke-web на сервере (<PROD_SERVER_IP>) имеет MTU bridge=1500, физический ens3=1450.
Java MinIO SDK внутри Docker зависает навсегда при обращении к <MINIO_IP>:9000 из-за дропа пакетов.
...
`minio-proxy` резолвится в `<IP_ADDRESS>` (Docker gateway = хост) через `extra_hosts` в `docker-compose-web.yml`.
...
**Правило выше теперь закреплено кодом .** Раньше бин `KaraokeStorageService` в karaoke-web
...
`deploy_karaokenet` больше нет, MinIO уехал на <MINIO_IP> → `UnknownHostException`). Приведено в
...
**Remote-хранилище из admin karaoke-app — прямой `MinioClient`, а не HTTP через прод-web .**
...
Она сломалась после переезда MinIO на <MINIO_IP> (прод-web до него не достучаться) — все remote
`exists`/`upload` отдавали **500**, HealthReport/`UPLOAD_TO_REMOTE_STORE` не работали. Теперь karaoke-app
(admin-машина) ходит в remote-хранилище **напрямую** через `MinioClient` к `<MINIO_IP>:9000` —
endpoint из новой property `storage.remote-endpoint` (env `STORAGE_REMOTE_ENDPOINT`, дефолт — новый
сервер), креды — те же `storage.key`/`storage.secret` (совпадают с локальными `***REMOVED***`/`***REMOVED***`).
...
свободно достают `<MINIO_IP>:9000` (MTU black-hole — проблема только прод-сервера <PROD_SERVER_IP>, не
...
**Актуализация счётчика HealthReport после async-задач + каскадное «Исправить всё» .**
...
- **Проблема 2 (умное «Исправить всё» — каскад по завершению).** Когда файла нет нигде, но он генерируемый:
 ...
 помечает песню в in-[memory] (`ConcurrentHashMap.newKeySet` — намеренно НЕ в
...
именно per-request `Connection.local()/remote()`. Фикс : во всех трёх контроллерах
...
`referer` в `tbl_events` годами показывал `<DOCKER_GATEWAY>` для всех событий — не баг nginx (прод
...
- **`ClientIpResolver`** (`karaoke-web/./util/ClientIpResolver.kt`) — читает `X-Forwarded-For`
 ...
 которого раньше не было вовсе. **`referer` для `callRest` переосмыслен :** раньше туда
...
- **Новый `event_type = "player"`** — события онлайн-плеера: `link_type` = `PlayerAction`
 ...
 `anonId` query-параметром. **`shown` (бывш. `open`, переименовано <DATE>) — это НЕ «пользователь
...
**Счётчики главной страницы (`GET /api/public/stats`, `karaoke-web/Stat.kt`, обновлено <DATE>).**
...
**Дашборд статистики webvue3 + расширенный трекинг .** Крупная доработка сбора и
...
**Страна по IP + источник перехода в статистике .** Два новых атрибута посетителя в
...
## Часть 2. Публичные секции (karaoke-public, онлайн-плеер, регистрация, редактор, избранное/плейлисты)
...
- Вся логика подсчёта кликов/окна/порога — **на бэкенде** (`karaoke-web`, `PlayerGestureUnlockService`,
 in-[memory], НЕ в БД — чтобы не тянуть это в LOCAL↔SERVER синхронизацию и не оставлять
...
.** В каждой строке таблиц перед иконкой спонсора — иконка плеера
...
**Редизайн таблиц «Закрома»/«Поиск» + премиум-монетка (karaoke-public, <DATE>, задеплоено на прод).**
...
**Выбор автора в «Закромах» плитками вместо `<select>` (karaoke-public, <DATE>, задеплоено на прод).**
...
`_getEndSequenceAlphas()`/`_getStartSequenceAlphas()` управляют фазой "fade-in сплэша".

**Экран загрузки: логотип + индикатор прогресса + стартовый переход logo→splash .**
Пока песня грузится (`!this._ready`) на canvas поверх процедурного звёздного фона рисуется логотип
(`_renderLogo`, alpha=1) и индикатор загрузки `_renderLoadingIndicator(ctx, W, H)` под ним (Y≈`H*0.72`).
Раньше это перекрывалось DOM-оверлеем `#kp-loading` с `background:#000` — теперь оверлей `display:none`/
`background:transparent` и служит **только для показа ошибок** (в `catch` внутри `init()` ему возвращают
`display:flex`; логотип остаётся виден за прозрачным фоном). Индикатор ведётся полем `this._loadProgress`:
- `null` — байт ещё нет (грузятся playerdata/шрифты/картинки): анимированный спиннер-дуга + «Загрузка.».
- `[0,1)` — реальный прогресс скачивания стемов: полоса (акцент `#f80`) + «N%».
- `>=1` — скачивание закончилось, идёт `decodeAudioData` (прогресса по нему нет): полоса стоит
 заполненной целиком с **мягкой пульсацией яркости** (синус по `performance.now()`) + «Обработка.».
Реальный прогресс скачивания даёт переписанный `_fetchAudio(url, onProgress)` — потоковое чтение тела
через `resp.body.getReader()` + заголовок `Content-Length`; при их отсутствии — фолбэк на `arrayBuffer()`
(прогресс остаётся `null`). `_loadAudio()` агрегирует байты **обоих** стемов в общий процент. Локально
(MinIO рядом) скачивание почти мгновенно, поэтому доминирует и дольше всего висит именно фаза «Обработка.»;
у посетителей по сети дольше видна полоса скачивания — это техническая реальность Web Audio API (по
декодированию прогресса нет), не баг.

Стартовый переход после готовности — `_getStartSequenceAlphas()`, зеркало `_getEndSequenceAlphas()`, но
**без hold**: `init()` при `_ready=true` пишет `this._startFadeStartedAt = Date.now()`, функция по
wall-clock проигрывает **0.5с fade-out логотипа → 0.5с fade-in сплэша** и обнуляет `_startFadeStartedAt`.
В `_renderFrame` он имеет **приоритет** над конечным: `const seq = this._getStartSequenceAlphas() ||
this._getEndSequenceAlphas()` (в любой момент активен максимум один), `seq.logoAlpha`/`seq.splashAlpha`
кормят `_renderLogo`/`_renderSplash`. Работает по wall-clock, т.к. воспроизведение ещё не идёт (`dt=0`,
сплэш статичен). Любое действие пользователя (`_play`/`_seekToDisplayTime`) и `_loadNewFile` обнуляют
`_startFadeStartedAt` — ровно как уже делалось для `_endFadeStartedAt`. Всё это — **в обеих копиях**
плеера (см. ниже про две копии `KaraokePlayer.js`).
...
с `KaraokeProperties` (~150 файловых настроек рендеринга, `/<RUNTIME_PATH>/Karaoke.properties`,
...
(<PROD_SERVER_IP>:8832) отдельно, миграция сама на сервер не попадает. То же самое для `ALTER TABLE`
...
(`set $karaoke_app_upstream .; proxy_pass $karaoke_app_upstream/api;`) + `resolver <IP_ADDRESS> valid=10s
...
табов/BPM/групп/MIDI). Реализовано и live-проверено на LOCAL <DATE>; миграция применена на LOCAL и PROD;
...
## «Избранное» и «Плейлисты» пользователей сайта + плейлист автора (задеплоено на прод <DATE>)
...
## Персистентность фильтров «Публикаций» + чистка секретов в do.sh 
...
контейнер `webvue3` пересобран и перезапущен. Детали.

**Утечка `DOCKER_PASSWORD` в вывод `do.sh`.** Скрипт (и три его копии — `~/<PROJECT>/deploy/do.sh`,
`~/<PROJECT>/deploy/new_comp/sm-karaoke-system/deploy/do.sh`, рантайм `/<RUNTIME_PATH>/deploy/do.sh`) при
каждом запуске делал `echo "DOCKER_PASSWORD = $DOCKER_PASSWORD"` — реальный Docker Hub access token попадал
в открытом виде в лог/лог (уже фиксировалось раньше как известный риск). По прямому
...
три `do.env` (`~/<PROJECT>/deploy/do.env`, `web-server-deploy/deploy/do.env`,
`new_comp/sm-karaoke-system/deploy/do.env`, все в `.gitignore`) и в серверный `Karaoke/deploy/do.env` на
<PROD_SERVER_IP> (по явному разрешению — правка через `sed -i` по SSH, без вывода значения в лог). Замены
