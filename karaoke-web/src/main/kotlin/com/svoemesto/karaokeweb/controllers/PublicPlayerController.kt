 ...
 *
 * ДЕМО-РЕЖИМ: если контент готов ([stemsReady] — персистентные флаги idStatus>=6 + оба стема
...
class PublicPlayerController(
    ...
    private fun isPremiumUser(request: HttpServletRequest): Boolean = siteUserResolver.resolve(request)?.isEffectivePremium == true
    ...
    private fun isSubscribedToSong(
    ...
    // Song.isContentReady, specs/083-auto-news-song-release).
    ...
    fun access(
        ...
        if (canWatch || isDemo) {
            ...
            val linkType =
                if (request.getParameter("source") == "list") {
                    PlayerAction.OPENED.dbValue
                } else {
                    PlayerAction.SHOWN.dbValue
                }
     ...
     *  - contentReady — премиум-независимая готовность контента ([stemsReady]: idStatus>=6 + оба
    ...
    fun readiness(
        ...
        val subscribedIds =
            if (premium) {
                ...
                Subscription.subscribedSongIds(userId, songIds, WORKING_DATABASE, storageService, storageApiClient)
            }
        val items =
    ...
    private fun existsInMinIO(storageKey: String): Boolean =
        try {
            val conn =
                java.net
                    .URL("$minioProxyUrl/minio/karaoke/${encodedProxyPath(storageKey)}")
    ...
    private fun fetchFromMinIO(storageKey: String): ByteArray? =
        try {
            val conn =
                java.net
                    .URL("$minioProxyUrl/minio/karaoke/${encodedProxyPath(storageKey)}")
    ...
    fun playerData(
        ...
        gestureUnlockService.assignmentIdForToken(token, id)?.let { assignmentId ->
            val assignment =
                SongAssignment
                    .getById(assignmentId, WORKING_DATABASE, storageService, storageApiClient)
                    ?.takeIf { it.songId == id }
