<template>
  <div class="chart-card">
    <div class="chart-head">
      ...
      <select class="form-select form-select-sm w-auto" :value="mode" @change="$emit('change-mode', $event.target.value)">
...
export default {
  ...
  emits: ['change-mode'],
  computed: {
    split() { return this.mode !== 'all' },
    chartHeight() { return this.mode === 'detail' ? 380 : 300 },
    categories() {
    ...
    series() {
      ...
      return this.seriesKeys.map(k => {
        const byDate = {}
        this.items.filter(i => i.eventType === k).forEach(i => { byDate[i.date] = i.count })
