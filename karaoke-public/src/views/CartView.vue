  ...
  <div class="km-page">
    ...
    <div class="km-content">
      <div class="km-title-row">
        ...
        <button v-if="rows.length > 0" class="km-btn km-btn-danger" @click="onClear">Очистить корзину</button>
      </div>
      ...
      <div v-else-if="rows.length === 0" class="km-empty">
        ...
        <RouterLink to="/zakroma">Закромах</RouterLink> или <RouterLink to="/filter">Поиске</RouterLink>.
      </div>
      <template v-else>
          ...
          <div v-for="row in rows" :key="row.songId" class="km-cart-card">
            <div class="km-cart-main">
              ...
              <div class="km-cart-price">
                ...
                <span v-if="row.personalDiscountPercent > 0" class="km-cart-promo km-cart-promo-personal">Ваша скидка {{ row.personalDiscountPercent }}%</span>
              </div>
            </div>
        ...
        <div class="km-cart-summary">
          <div class="km-cart-total">Итого: <b>{{ total }} ₽</b></div>
          ...
          <button
            ...
            :disabled="!disclaimerAccepted || checkingOut || (total > 0 && paymentsEnabled === false)"
            @click="onCheckout"
          >
...
export default {
  ...
  methods: {
    async load() {
      ...
      } catch (e) { /* оставляем пустой список */ }
      this.loading = false
    },
    ...
    async onCheckout() {
      ...
      try {
        ...
        this.checkoutErrorText = (body && body.error === 'cart_empty')
...
.km-title { font-size: 1.4rem; margin: 0; }
.km-title-row { display: flex; align-items: center; justify-content: space-between; gap: 1rem; margin-bottom: 1.25rem; }
.km-hint, .km-empty { font-size: 0.9rem; color: var(--km-text2); }
.km-empty a { color: var(--km-accent); }

.km-cart-list { display: flex; flex-direction: column; gap: 0.75rem; margin-bottom: 1.25rem; }
.km-cart-card {
  display: flex; justify-content: space-between; align-items: center; gap: 1rem;
  background: var(--km-card); border: 1px solid var(--km-border); border-radius: 12px; padding: 1rem 1.25rem;
}
.km-cart-title { font-weight: 600; font-size: 0.95rem; }
.km-cart-price { display: flex; align-items: baseline; gap: 0.4rem; margin-top: 0.25rem; font-size: 0.85rem; }
.km-cart-price-old { text-decoration: line-through; color: var(--km-text2); }
.km-cart-price-final { font-weight: 700; }
.km-cart-promo { font-size: 0.72rem; background: linear-gradient(135deg, #f6c94b, #d99413); color: #5a3c00; padding: 0.1rem 0.45rem; border-radius: 10px; }
.km-cart-promo-personal { background: linear-gradient(135deg, #a78bfa, #7C3AED); color: #fff; }

.km-cart-summary {
  background: var(--km-card); border: 1px solid var(--km-border); border-radius: 14px; padding: 1.5rem;
}
.km-cart-total { font-size: 1.1rem; margin-bottom: 1rem; }
.km-disclaimer-label { display: flex; gap: 0.5rem; align-items: flex-start; font-size: 0.78rem; line-height: 1.35; color: var(--km-text2); margin-bottom: 1rem; cursor: pointer; }
.km-disclaimer-label input { margin-top: 0.2rem; flex-shrink: 0; }
.km-error { font-size: 0.85rem; color: #e05555; margin-bottom: 0.75rem; }

.km-btn { display: inline-block; border-radius: 8px; padding: 0.5rem 1.1rem; font-weight: 600; text-decoration: none; cursor: pointer; border: none; font-size: 0.88rem; white-space: nowrap; }
