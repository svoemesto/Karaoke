...
- 2026-07-26: добавлен FR-011 (кнопки быстрой вставки тегов) по прямому вопросу пользователя — реализовано в `webvue3` и `karaoke-public`.
- 2026-07-26: добавлен FR-012 (форс-алаймент сервис ДОЛЖЕН согласованно с backend считать слоги на тексте со спецтегами) — по факту реального бага, воспроизведённого пользователем на проде ("Число слогов в тексте не совпало с результатами выравнивания"). Исправлено в `alignment-ml/syllables.py` (четвёртое независимое место, не входившее в исходный scope — см. `research.md` §9). Повторная проверка чек-листа — пройдено; FR-012 измерим и тестируем (unit-проверка на фикстурах, см. `quickstart.md`).
