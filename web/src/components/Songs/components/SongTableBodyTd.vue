<template>
  <div :style="styleRoot">
    <div :id="'song-'+id+'-'+name" :style="styleVal" @dblclick="$emit('dblclick')" @click.left="$emit('click')" v-text="text">
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    textAlign() {
      return this.$store.getters.getSongFieldParams(this.name).textAlign;
    },
    fontSize() {
      return this.$store.getters.getSongFieldParams(this.name).fontSize;
    },
    width() {
