  ...
  <div class="se-overlay" @click.self="$emit('close')">
    <div class="se-modal">
      ...
      <div class="se-field">
        ...
        <div v-if="selectedSongs.length > 0" class="se-selected-block">
          <div class="se-selected-chips">
            <div v-for="s in selectedSongs" :key="s.id" class="se-chip">
              <span
                >#{{ s.id }} · {{ s.author }} <template v-if="s.album">— {{ s.album }}</template> —
                {{ s.songName }}</span
              >
              <button
                type="button"
                class="se-chip-remove"
          ...
          <button type="button" class="se-btn se-btn-clear-all" @click="selectedSongs = []">
            Очистить всё ({{ selectedSongs.length }})
          </button>
        </div>
        ...
        <div v-else-if="!searched" class="se-search-message">
          Введите критерии и нажмите «Найти»
        </div>
        ...
        <template v-else>
          ...
          <ul class="se-result-list">
            ...
            >
              ...
              <button type="button" class="se-btn se-btn-hr-mini" @click.stop="hrSongId = s.id">
                HR
              </button>
            </li>
          </ul>
          ...
          {{
            busy
              ...
              : selectedSongs.length > 1
                ? `Назначить (${selectedSongs.length})`
                : 'Назначить'
          }}
        </button>
      </div>
...
export default {
  ...
  computed: {
    ...
    canSubmit() {
      return this.assigneeId > 0 && this.selectedSongs.length > 0
    },
  },
  mounted() {
  ...
  methods: {
    ...
    async doAssign() {
      ...
      for (const s of toAssign) {
        try {
          const res = await this.$store.dispatch('assignSong', {
            songId: s.id,
      ...
      if (failed.length === 0) {
        ...
        this.message =
          (succeeded.length > 0 ? `Назначено ${succeeded.length} из ${toAssign.length}. ` : '') +
          'Не удалось: ' +
          failed.map((f) => `#${f.song.id} — ${this.errorText(f.error)}`).join('; ')
      }
    },
    errorText(code) {
      return (
...
.se-result-row-selected {
  background: #f3fbf3;
}
.se-result-row-selected:hover {
  background: #e8f7ea;
}
.se-btn-hr-mini {
