...
`siteusers`, `events`, `siteplaylists`, `siteplaylistitems`, `songassignments`, `songassignmentdrafts`,
`pricetariffs`, `subscriptions`) через generic-функцию `collectSyncOps()`. Разрешения — **8 флагов на сущность**
...
MonitorCheck` (сейчас 4: горизонт запланированных постов в Telegram, доступность прод-сервера,
остановленная очередь рендера, выключенный Telegram-поллинг; добавление новой проверки — одна
строка в реестре). `MonitoringService` (`@Scheduled` раз в минуту) прогоняет их, хранит снапшот в
...
- **karaoke-web Settings trap.** В `karaoke-web` **нельзя** трогать `Settings.rootFolder` / любые `*NameFlac`-
  ...
  emptyList() }` (важно `Throwable`, не `Exception` — `NoClassDefFoundError` это `Error`). Детали → memory
...
- **Онлайн-редактор разметки (`/account/editor`) с модерацией.** Упрощённый публичный аналог `SubsEdit.vue`:
  ...
  **обязательно** на бэкенде (`PublicSongEditorController`, иначе снятие роли не отзывает доступ к уже
  ...
  подтверждение на их очистку В ЗАДАНИИ (не в самой песне). Детали →
