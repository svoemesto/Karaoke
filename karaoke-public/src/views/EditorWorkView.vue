  ...
  <div class="km-page ke-page">
        ...
        <div class="ke-kb-grid">
          <div v-for="(row, ri) in keyboardRows" :key="ri" class="ke-kb-row">
            <button
              ...
              :class="{
                'ke-kb-key-active': k.code && heldKeys[k.code],
                'ke-kb-key-inactive': !k.code,
                'ke-kb-key-spacer': k.spacer,
              }"
    ...
    <footer v-if="!loading && !loadError && canEdit" class="ke-footer ke-sticky-bottom">
      ...
      <div class="ke-footer-btns">
        <button class="ke-btn ke-btn-ghost" :disabled="saveState === 'saving'" @click="saveNow">
          Сохранить
        </button>
        <button
          class="ke-btn ke-btn-primary"
          :disabled="submitting || !hasAnyMarkers"
          @click="submit"
        >
...
export default {
  ...
  computed: {
    ...
    playerSrc() {
      return this.task ? `/player/${this.task.songId}` : ''
    },
    saveStateLabel() {
      return (
  ...
  async mounted() {
    ...
    if (!this.token) {
      this.$router.push({
        path: '/login',
        query: { redirect: `/account/editor/${this.$route.params.id}` },
      })
      return
    }
  ...
  methods: {
    ...
    async load() {
      ...
      const rawMarkersPerVoice =
        body.markersPerVoice && body.markersPerVoice.length ? body.markersPerVoice : [[]]
    ...
    removeLastVoice() {
      ...
      if (!confirm(`Удалить голос ${this.voices.length}? Весь его текст и маркеры будут потеряны.`))
        return
      this.voices.pop()
...
.ke-kb-key-active .ke-kb-key-label,
.ke-kb-key-active .ke-kb-key-caption {
  color: #fff;
}
...
.ke-kb-key-spacer .ke-kb-key-label {
  font-size: 0.8rem;
}
...
.ke-preview :deep(.ke-fx-cur) {
  color: #ff0000;
  font-weight: bolder;
}
.ke-preview :deep(.ke-fx-group0) {
  color: #ffffff;
  font-weight: bolder;
}
.ke-preview :deep(.ke-fx-group1) {
  color: #ffff00;
  font-style: italic;
  font-weight: bolder;
}
.ke-preview :deep(.ke-fx-group2) {
  color: #00bfff;
  font-weight: bolder;
}
.ke-preview :deep(.ke-fx-group3) {
  color: #00ff00;
  font-style: italic;
  font-weight: bolder;
}
.ke-preview :deep(.ke-fx-comment) {
  color: #d2691e;
  font-size: 0.78em;
  font-style: italic;
  font-weight: bolder;
