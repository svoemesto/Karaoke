  ...
  <div class="km-page">
    ...
    <div class="km-content">
          ...
          <template v-if="selectedTariffId">
            ...
            <template v-else-if="priceInfo">
              <div class="km-price-row">
                <span v-if="priceInfo.discount > 0" class="km-price-old"
                  >{{ priceInfo.base }} ₽</span
                >
                <span class="km-price-final">{{ priceInfo.final }} ₽</span>
                <span v-if="priceInfo.promoApplied" class="km-promo-badge">{{
              ...
              <label class="km-checkbox-label">
                ...
                <span>
                  ...
                  мне не переходят. Доступ — для личного некоммерческого использования, без передачи
                  файлов и без скачивания. Полные условия — в
      ...
      <div class="km-card">
        ...
        <p class="km-card-body">
          Не нужна подписка на весь сайт? На странице понравившейся песни (если она доступна для
          подписки) есть кнопка «Оформить подписку на эту песню» — бессрочный доступ без переплаты.
        </p>
      ...
      <div class="km-card">
        ...
        <p class="km-card-body">
          Оформите платную подписку на
          <a href="https://sponsr.ru/smkaraoke" target="_blank" rel="noopener"
            >sponsr.ru/smkaraoke</a
          >. После оплаты укажите свой Sponsr-профиль в
          <RouterLink to="/account">личном кабинете</RouterLink> (поле «Sponsr UID») — мы сверим
          активных подписчиков и включим премиум автоматически (обычно в течение суток).
        </p>
        <a
          href="https://sponsr.ru/smkaraoke"
          target="_blank"
          rel="noopener"
    ...
    async loadSongTariff() {
      ...
      try {
        const { status, body } = await authGet(
          '/api/public/account/subscription/tariffs?scope=SONG',
          '',
        )
        ...
        /* карточка просто не покажет цену */
      }
