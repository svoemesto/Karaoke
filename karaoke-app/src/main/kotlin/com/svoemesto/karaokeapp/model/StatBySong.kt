...
object StatsByEvents {
        ...
        try {
            ...
            rs =
                statement.executeQuery(
                    "select count(*) as cnt from tbl_events where ${buildEventsWhere(eventType, fromDays, siteUserId, anonId, songId)}",
                )
    ...
    fun getWebEvents(
        ...
        val sql =
            """
            select * from tbl_events where ${buildEventsWhere(
                eventType,
                fromDays,
                siteUserId,
                anonId,
                songId,
            )} order by last_update desc, id desc limit $limit offset $offset
            """.trimIndent()
    ...
    private fun webEventHuman(
        ...
        when (r.eventType) {
            EventType.CALL_REST.dbValue ->
                when (r.restName) {
                    ...
                    RestName.ZAKROMA.dbValue ->
                        "Закрома" to
                            (if (r.restParameters.contains("=")) r.restParameters.substringAfter("=").trimEnd('}') else "")
            ...
            EventType.PLAYER.dbValue ->
                detailLabel("player", r.linkType) to
    ...
    fun getTopListenedSongs(
        database: KaraokeConnection = WORKING_DATABASE,
        limit: Int = 50,
        offset: Int = 0,
    ): List<TopListenedSongDto> {
    ...
    private fun channelLabel(linkName: String?): String =
        when (linkName) {
            "vk" -> "VK"
            "dzen" -> "Dzen"
            "tg" -> "Telegram"
    ...
    fun getSummary(database: KaraokeConnection = WORKING_DATABASE): StatsSummaryDto {
        ...
        val uniqueVisitors =
            scalarInt(database, "select count(distinct anon_id) from tbl_events where anon_id is not null and anon_id <> ''")
    ...
    fun getEventsByType(
        ...
        try {
            ...
            rs =
                statement.executeQuery(
                    "select event_type, count(*) as cnt from tbl_events where $where group by event_type order by cnt desc",
                )
            while (rs.next()) {
    ...
    fun getTopUsersCount(database: KaraokeConnection = WORKING_DATABASE): Int =
        scalarInt(database, "select count(distinct site_user_id) from tbl_events where site_user_id > 0") +
            scalarInt(
                database,
                "select count(distinct anon_id) from tbl_events where site_user_id = 0 and anon_id is not null and anon_id <> ''",
            )
