  ...
  <div class="set-table">
    ...
    <div class="set-toolbar">
      ...
      <input
        ...
        class="set-toolbar-item"
        type="text"
        placeholder="Автор"
        list="set-authors-list"
        @change="reload"
      />
    ...
    <div v-if="isRemoteView" class="set-remote-note">
      Просмотр серверной БД. Назначить/одобрить/отклонить/удалить — работают и отсюда (id совпадает
      в обеих БД); новое назначение создастся сразу на сервере (реальный цикл работы часто идёт
      целиком на PROD). Изменение самой песни при апруве в любом случае применяется в локальной БД.
    </div>
      ...
      >
        <template #table-busy>
          <div class="text-center text-danger my-2">
            <b-spinner class="align-middle" /><strong> Загрузка...</strong>
          </div>
        </template>
        ...
        <template #cell(status)="data">
          <span class="set-badge" :class="`set-badge-${data.value}`">{{
            statusLabel(data.value)
          }}</span>
        </template>
        <template #cell(actions)="data">
          <button class="set-mini-btn" @click.stop="openReview(data.item.id)">Просмотр</button>
          <button class="set-mini-btn set-mini-revoke" @click.stop="onRevoke(data.item)">
            Отозвать
          </button>
          <button class="set-mini-btn set-mini-del" @click.stop="onDelete(data.item.id)">
            Удалить
          </button>
        </template>
      </b-table>
...
export default {
  ...
  computed: {
    digest() {
      return this.$store.getters.getAssignmentsDigest || []
    },
    sortedDigest() {
    ...
    target: {
      get() {
        return this.$store.getters.getAssignmentsTarget
      },
      set(v) {
        this.$store.dispatch('setAssignmentsTarget', v)
      },
    },
    ...
    fields() {
      return [
        ...
        },
        {
          ...
          sortable: true,
          label: 'Автор',
          style: { minWidth: '160px', textAlign: 'left', fontSize: 'small' },
        },
        {
          key: 'year',
          sortable: true,
        ...
        },
        {
          ...
          sortable: true,
          label: 'Песня',
          style: { minWidth: '220px', textAlign: 'left', fontSize: 'small' },
        },
        {
          key: 'assigneeName',
          sortable: true,
          label: 'Редактор',
          style: { minWidth: '160px', textAlign: 'center', fontSize: 'small' },
        },
        {
          key: 'status',
          sortable: true,
  ...
  watch: {
    assignmentsIsLoading() {
      this.isBusy = this.$store.getters.getAssignmentsIsLoading
    },
  },
  async mounted() {
    ...
    this.$store.getters.songAuthorsPromise
      .then((data) => {
  ...
  methods: {
    ...
    reload() {
      ...
      this.$store.dispatch('loadAssignmentsDigest', params).finally(() => {
        this.isBusy = false
      })
    },
    async openReview(id) {
    ...
    async onRevoke(item) {
      if (
        !confirm(
          `Отозвать назначение у «${item.assigneeName || item.assigneeEmail}» на песню «${item.songName}»? Задание и его черновик будут удалены — редактор больше не сможет его править, и эту же песню сразу можно будет назначить другому через селектор «Назначить…» в таблице песен.`,
        )
      )
        return
...
.set-table-body :deep(th:hover svg.bi) {
