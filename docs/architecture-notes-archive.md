...
> **Что это.** Этот файл — вынесенная из `CLAUDE.md` подробная летопись реализованных фич и
> отловленных багов/«ловушек» (в основном датированные записи 2026-07-04…07). Он **не загружается
> автоматически** в контекст Claude Code (в отличие от `CLAUDE.md`) — открывать вручную (Read/grep),
> когда нужны детали конкретной фичи или ловушки.
>
> **Зачем вынесено.** `CLAUDE.md` разрастался до ~230 КБ и грузился целиком каждую сессию. Durable-
> карта архитектуры, команды сборки/деплоя и ключевые инварианты остались в `CLAUDE.md`; сюда
...
> `~/.claude/projects/-home-nsa-Karaoke/memory/` (индекс — `MEMORY.md`). Memory подтягивается on-demand
> по описанию; этот архив — полный текст-первоисточник и бэкап. Git-история (`git log`/`blame` по
> `CLAUDE.md`) хранит те же записи как коммиты.
>
...
**Экран загрузки: логотип + индикатор прогресса + стартовый переход logo→splash (2026-07-06).**
