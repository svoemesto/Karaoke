...
- **`Zakroma.getZakromaBySpecialOrder(database, storageService, storageApiClient)`** (новое) —
  ...
  Внутри альбом группируется по паре (год, название), а не только по названию — см.
  «Известные ловушки» и specs/018-fix-album-name-year-grouping.
...
- **Схлопывание одноимённых альбомов разных годов** (баг, specs/018-fix-album-name-year-grouping):
  `buildFromSettings()` группировал альбомы внутри автора только по названию
  (`groupBy { it.album }`). Если у автора есть два РАЗНЫХ альбома с одинаковым названием, но
  разными годами (переиздание и т.п.), все их песни схлопывались в одну карточку с годом
  «первой попавшейся» песни — второй альбом визуально исчезал вместе со своими песнями.
  Исправлено группировкой по паре (год, название) — это соответствует реальной идентичности
  альбома в БД (`tbl_albums_author_year_name_key`: уникальность по author_id+year+name).
