<template>
  <div class="sut-table">
    ...
    <div class="sut-toolbar">
      ...
      <button class="sut-toolbar-item sut-btn" @click="isFilterVisible = true">
    ...
    <div class="sut-table-body">
      ...
      >
        ...
        <template #cell(banned)="data">
          <div
            :style="{
              color: data.value ? 'darkred' : 'darkgreen',
              textAlign: 'center',
            }"
          >
...
export default {
  ...
  computed: {
    ...
    siteUsersDigest() {
      return this.$store.getters.getSiteUsersDigest
    },
    countRows() {
      return this.siteUsersDigest ? this.siteUsersDigest.length : 0
    },
    target: {
      get() {
      ...
      set(value) {
        this.$store.dispatch('setSiteUsersTarget', value)
      },
    },
    activeFilterCount() {
      return [
    ...
    siteUserDigestFields() {
      return [
        ...
        {
          key: 'effectivePremium',
          label: 'Активный премиум',
          sortable: true,
          style: {
            minWidth: '90px',
            maxWidth: '90px',
        ...
        {
          key: 'permanentPremium',
          label: 'Постоянный премиум',
          sortable: true,
          style: {
            minWidth: '90px',
            maxWidth: '90px',
        ...
        {
          key: 'sitePremiumUntil',
          label: 'Премиум сайта до',
          sortable: true,
          style: {
            minWidth: '110px',
            maxWidth: '110px',
  ...
  watch: {
    ...
    // Хардкод currentPage=1 в onTargetChange (ниже) — отдельный случай смены target, watcher
    // покрывает случай смены фильтра. Не удалять onTargetChange.
  ...
  methods: {
    reload() {
      ...
      if (this.$store.getters.getSiteUsersFilterId)
        params.filterId = this.$store.getters.getSiteUsersFilterId
      if (this.$store.getters.getSiteUsersFilterEmail)
        params.filterEmail = this.$store.getters.getSiteUsersFilterEmail
      if (this.$store.getters.getSiteUsersFilterDisplayName)
        params.filterDisplayName = this.$store.getters.getSiteUsersFilterDisplayName
      if (this.$store.getters.getSiteUsersFilterSponsrUid)
        params.filterSponsrUid = this.$store.getters.getSiteUsersFilterSponsrUid
      if (this.$store.getters.getSiteUsersFilterIsPremium)
        params.filterIsPremium = this.$store.getters.getSiteUsersFilterIsPremium
      if (this.$store.getters.getSiteUsersFilterIsPermanentPremium)
        params.filterIsPermanentPremium = this.$store.getters.getSiteUsersFilterIsPermanentPremium
      if (this.$store.getters.getSiteUsersFilterIsEffectivePremium)
        params.filterIsEffectivePremium = this.$store.getters.getSiteUsersFilterIsEffectivePremium
      if (this.$store.getters.getSiteUsersFilterIsEditor)
        params.filterIsEditor = this.$store.getters.getSiteUsersFilterIsEditor
      if (this.$store.getters.getSiteUsersFilterIsBanned)
        params.filterIsBanned = this.$store.getters.getSiteUsersFilterIsBanned
...
.sut-table-body :deep(th:hover svg.bi) {
