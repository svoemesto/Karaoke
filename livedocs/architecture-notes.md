...
**Связанные документы:** [`docs/features/news-publish-backfill.md`](./archive/docs/features/news-publish-backfill.md) — новый per-feature документ с пошаговой инструкцией. Pass 33 (PR feat 122, автопремиум-публикация) — причина появления этой фичи. Pass 9 (PR feat 101, флаг «доступна для новости») — пример точно такого же backfill-паттерна для единственного флага `newsAvailableAnnounced` (этот раз обобщили на 7-флаговый complete-набор).
...
**Связанные документы:** [`specs/125-news-flags-backfix/spec.md`](../specs/125-news-flags-backfix/spec.md) — спека с полным описанием fix. [`docs/features/news-publish-backfill.md`](../archive/docs/features/news-publish-backfill.md) — обновлён per-feature документ. Pass 34 (PR feat 124) — баг, который исправлен этой фичей. Бэкфилл-часть Pass 34 рабочая (на admin-машине); архитектурный баг был только в kill-switch и его endpoint.
...
**Связанные документы:** [`specs/128-news-publish-templates/spec.md`](../specs/128-news-publish-templates/spec.md) — спецификация. [`docs/features/news-templates.md`](../archive/docs/features/news-templates.md) — новый per-feature документ (18-я подсистема). Pass 35 (kill-switch через `tbl_public_settings`) — архитектурный прецедент для хранения prod-настроек в Postgres вместо KaraokeProperties. Pass 9 (`tbl_news.title`/`body`, флаг `newsAvailableAnnounced`) — оригинальная архитектура auto-новостей. ВК/Telegram-аналоги (Pass 28, 31) — образец `nav-tabs` UI-паттерна.
...
- [`docs/features/songs-table.md`](../archive/docs/features/songs-table.md) — обновлённый per-feature документ.
...
- [docs/features/approve-pipeline.md](../archive/docs/features/approve-pipeline.md) — добавлена секция «Условный запуск при выборе статуса 5».
- [docs/features/editor-tasks.md](../archive/docs/features/editor-tasks.md) — добавлена секция «Выбор статуса при апруве».
...
- [docs/features/zakroma-stream-progress.md](../archive/docs/features/zakroma-stream-progress.md) — добавлена секция «Pass 52 (186) — ускорение загрузки крупных авторов».
...
- [docs/features/site-traffic-resilience.md](../archive/docs/features/site-traffic-resilience.md) — новый per-feature документ.
...
- ✅ FR-109/H-12: EventsBuffer для batch INSERT в `tbl_events` (PR #389, спека
  `274-events-batch-insert`). Новый `@Service EventsBuffer` буферизует события через
  `ConcurrentLinkedQueue<EventRecord>` + `@Scheduled flush` (дефолт 5 сек) + JDBC
  `addBatch`/`executeBatch`. Kill-switch `karaoke.web.events.batch-enabled` (default **false**
  — opt-in для безопасного rollout). Backpressure при переполнении (>500) — flush немедленно.
  Fail-open при ошибке (буфер очищается, потеря допустима для логирования). MainController.insertEvent
  → eventsBuffer.add(EventRecord(...)). Эффект: ≥80% снижение RPS INSERT при включении.
...
- Tier-3: `pg_stat_statements`-инструмент (FR-108, deferred из Clarifications Session 2026-08-26).
