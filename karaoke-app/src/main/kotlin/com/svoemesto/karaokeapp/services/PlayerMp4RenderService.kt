...
object PlayerMp4RenderService {
     ...
     */
    fun renderFrames(
        params: RenderMp4Params,
        onProgress: ((Int) -> Unit)? = null,
    ): RenderMp4FramesResult {
        val framesDir = File(PATH_TO_TEMP_RENDERMP4_FOLDER, "${params.songId}_${params.version.name}")
        framesDir.deleteRecursively()
        framesDir.mkdirs()

        log(
            "Открываем headless-плеер для песни ${params.songId} (${params.width}x${params.height}@${params.fps}) version=${params.version.name}",
        )
