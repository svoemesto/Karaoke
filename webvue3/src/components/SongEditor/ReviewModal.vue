    ...
    <div class="se-modal se-modal-wide" @click.stop>
      <template v-if="a">
        ...
        <div class="se-player-toggle">
          ...
          <button type="button" class="se-btn se-btn-primary" @click="$emit('open-editor', { assignmentId: a.id, songId: a.songId, target: targetForEditor })">
            ▶ Открыть в редакторе
          </button>
        </div>
...
export default {
  ...
  data() {
    return { comment: '', busy: false, message: '', isError: false, showPlayer: false, currentVoiceIdx: 0, playerHeight: 0 }
  },
  computed: {
    ...
    targetForEditor() { return this.$store.getters.getAssignmentsTarget || 'local' },
...
.se-modal { background: #fff; border-radius: 12px; padding: 1.5rem; width: 420px; max-width: 92vw; display: flex; flex-direction: column; gap: 0.8rem; font-family: Avenir, Helvetica, Arial, sans-serif; font-weight: 400; box-sizing: border-box; }
...
.se-player-wrap { width: 100%; border-radius: 8px; overflow: hidden; background: #000; }
.se-player-frame { width: 100%; border: none; display: block; }
.se-modal-title { margin: 0; font-size: 1.15rem; }
.se-dim { color: #888; font-weight: 400; }
.se-meta { display: flex; flex-wrap: wrap; gap: 0.4rem 1.2rem; font-size: 0.82rem; color: #555; font-weight: 400; }
...
.se-col-title { font-size: 0.72rem; text-transform: uppercase; color: #888; font-weight: 400; margin-bottom: 0.3rem; }
.se-text { background: #f5f5f5; border-radius: 8px; padding: 0.6rem; font-size: 0.82rem; max-height: 220px; overflow: auto; white-space: pre-wrap; margin: 0; font-weight: 400; }
.se-marker-summary { background: #f5f5f5; border-radius: 8px; padding: 0.6rem; font-size: 0.85rem; display: flex; flex-direction: column; gap: 0.3rem; font-weight: 400; }
.se-prev-comment { font-size: 0.8rem; color: #a9500f; background: #fff3e8; border-radius: 8px; padding: 0.4rem 0.6rem; font-weight: 400; }
.se-remote-note { font-size: 0.8rem; color: #8a6d0a; background: #fef8e3; border: 1px solid #f2dd9a; border-radius: 8px; padding: 0.5rem 0.6rem; font-weight: 400; }
.se-field { display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.85rem; font-weight: 400; }
.se-field textarea { padding: 0.45rem 0.6rem; border: 1px solid #ccc; border-radius: 8px; font-size: 0.9rem; resize: vertical; font-weight: 400; }
.se-msg { font-size: 0.85rem; color: #2a7a3a; margin: 0; font-weight: 400; }
.se-msg-err { color: #c0392b; }
.se-modal-btns { display: flex; justify-content: flex-end; gap: 0.6rem; margin-top: 0.5rem; }
.se-btn { border: 1px solid #bbb; border-radius: 8px; padding: 0.45rem 1rem; background: antiquewhite; cursor: pointer; font-size: 0.9rem; font-weight: 400; }
...
.se-loading { padding: 2rem; text-align: center; color: #888; font-weight: 400; }
