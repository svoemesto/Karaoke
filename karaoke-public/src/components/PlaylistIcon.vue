    ...
    <div v-if="open" class="pl-menu-backdrop" @click="close">
      <div class="pl-menu" :style="menuStyle" @click.stop>
        ...
        <template v-if="inPlaylists.length">
          <div v-for="pl in inPlaylists" :key="'in-' + pl.id" class="pl-group">
            ...
            <div v-if="expandedId === pl.id" class="pl-sub">
              ...
              <button
                class="pl-item pl-sub-item pl-danger"
                :disabled="busy"
                @click="removeFrom(pl.id)"
              >
        ...
        <div v-if="showAdd" class="pl-sub">
          <button class="pl-item pl-sub-item pl-accent" :disabled="busy" @click="addToNew">
            ➕ В новый плейлист
          </button>
          <button
...
export default {
  ...
  setup(props) {
    ...
    const currentIds = computed(
      () => (membership.membership[String(props.songId)] || {}).playlistIds || [],
    )
    const inPlaylists = computed(() =>
      myPlaylists.value.filter((p) => currentIds.value.includes(p.id)),
    )
    const notInPlaylists = computed(() =>
      myPlaylists.value.filter((p) => !currentIds.value.includes(p.id)),
    )
    ...
    function setLocalIds(ids) {
      membership.setPlaylistIds(props.songId, ids)
    }
    ...
    function goTo(id) {
      close()
      router.push({ path: `/account/playlists/${id}` })
    }
...
.pl-sub {
  padding-left: 10px;
  border-left: 2px solid var(--km-border, #e2e2e2);
...
.pl-divider {
  height: 1px;
  background: var(--km-border, #e2e2e2);
  margin: 5px 4px;
}
.pl-empty {
  font-size: 0.78rem;
