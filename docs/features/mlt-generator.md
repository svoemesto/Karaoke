...
сплэш, водяной знак, фон). Принимает `Song` (песня + параметры) и
...
1. На вход: `Song` (одна песня) + `KaraokeProperties` (глобальные
...
- **MUST**: MLT-генератор НЕ лезет в сеть — все данные берёт из `Song`
  + `KaraokeProperties` + локальной БД. Self-contained.
- **MUST**: `Song` всегда имеет `songType` ∈ {song, instrumental, poetry} —
...
- **Длинные строки текста** — `MkoSongText` разбивает на слоги по `Song.lyricsText`,
...
- **Аудио-синхронизация** — `melt` стартует с первого аудио-продьюсера.
  Если в `Song` несколько аудио (минус + оригинал), первый — тот, у
  кого меньше `offset`.
