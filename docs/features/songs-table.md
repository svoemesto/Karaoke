...
**Доработка в #160 (2026-08-06)**: 6 узких цветовых колонок публикации (`.publish-column`) удалены из `PublishTableBodyTd.vue`; ячейка `.publish-name` расширена со 150 px до 210 px (6 × 10 px). Снята раскраска 4 кнопок «PLAY LYRICS / KARAOKE / CHORDS / TABS» в `SongEdit.vue` (inline `:style="{ backgroundColor: song.processColorMelt* }"` удалён). **DTO почищены**: в `SongDTO` и `SongDTOdigest` удалены 27 из 28 полей `processColor*`, осталось ровно одно — `processColorPlayerDemo` (живой потребитель: бейдж `DE` в `SongsTable.vue:329`). Соответствующие 27 присваиваний удалены в `Song.toDTO()` и `SongDTO.toDtoDigest()`. **Геттеры в `Song.kt` (2454–2538) и diff-логика LOCAL↔SERVER (~6816+) сохранены** (Принцип III NON-NEGOTIABLE): продолжают использоваться `Publication.kt` для собственных геттеров `publishXcolorMeltY` и серверными шаблонами `publications.html`/`unpublications.html`/`songs.html`/`songs2.html`/`area_left_column.html` через `${song.processColorX}` и JS `data.processColorX` после `fetch('/song/{id}')`. Серверные шаблоны получают raw `Song` через `MainController.getSong` (не DTO) и работают без изменений. Эффект: payload `/api/songsdigests` сокращается на ~5 МБ (18 858 песен × 27 полей × ~10 байт).
