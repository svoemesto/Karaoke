...
4. **Дообучение (опционально)** — `train.py`, только если `evaluate.py` показал, что baseline
   ...
   текст) — принудительно режется по `--max-chunk-ms` (по умолчанию 20с — снижено с 30с ради запаса
   по видеопамяти, см. ниже), просто как потолок длины.
   ...
   Чекпоинт пишется каждые `--save-steps` шагов (по умолчанию 200), не только по эпохам — можно
   безопасно прервать обучение (Ctrl+C) и продолжить с того же места флагом `--resume` (см. ниже).
   Полный eval по оценочной выборке гоняется отдельно, каждые `--eval-steps` (по умолчанию 1000) —
   специально НЕ привязано к `--save-steps`: eval заметно дороже самой записи чекпоинта, часто
   чекпоинтиться не значит часто оценивать. Прогресс (шаг/всего, %, loss, eval_loss, прошло/осталось
   времени) печатается каждые `--logging-steps` (по умолчанию `--save-steps`/10) отдельными строками,
   а не перезаписываемым на месте tqdm-баром — удобно смотреть через `tail -f`, если процесс запущен
   в фоне.
