<template>
  <div class="sps-wrapper">
    ...
    <div v-if="status" class="sps-status">
      <div>
        URL страницы подписчиков (sponsrSubscribersUrl):
        <b :class="status.subscribersUrlConfigured ? 'sps-ok' : 'sps-warn'">
          {{ status.subscribersUrlConfigured ? 'настроен' : 'НЕ настроен' }}
        </b>
        <span class="sps-hint"
          >(настраивается в разделе «Настройки» — свойство sponsrSubscribersUrl)</span
        >
      </div>
      <div>
        Сохранённая сессия Sponsr:
        <b :class="status.sessionSaved ? 'sps-ok' : 'sps-warn'">{{
          status.sessionSaved ? 'есть' : 'нет'
        }}</b>
        <span class="sps-hint"
          >(создаётся вручную вызовом createNewSponsrAuthContext() на admin-машине)</span
        >
      </div>
      <div>
        Окно продления: {{ status.syncWindowDays }} дн.
        <span class="sps-hint">(свойство sponsrSyncWindowDays)</span>
      </div>
    </div>
    ...
    <div class="sps-section">
      ...
      />
      <button class="sps-btn" :disabled="isLoading || !identifiersText.trim()" @click="doImport">
        Импортировать
      </button>
    </div>
    ...
    <div class="sps-section">
      ...
      <div class="sps-hint">
        Работает только при настроенном URL и сохранённой сессии (см. статус выше).
      </div>
    ...
    <div v-if="lastResult" class="sps-result" :class="{ 'sps-result-fail': !lastResult.ok }">
      <div>
        Найдено идентификаторов: {{ lastResult.foundIdentifiers }}, сопоставлено пользователей:
        {{ lastResult.matchedUsers }}
      </div>
      <ul>
...
export default {
  ...
  computed: {
    status() {
      return this.$store.getters.getSponsrSyncStatus
    },
    isLoading() {
      return this.$store.getters.getSponsrSyncIsLoading
    },
    lastResult() {
      return this.$store.getters.getSponsrSyncLastResult
    },
    target: {
      get() {
        return this.$store.getters.getSponsrSyncTarget
      },
      set(v) {
        this.$store.dispatch('setSponsrSyncTarget', v)
      },
    },
  },
  mounted() {
