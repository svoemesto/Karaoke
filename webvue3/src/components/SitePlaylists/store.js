...
export default {
    ...
    actions: {
        ...
        loadSitePlaylistDetail(ctx, id) {
            const request = { method: 'POST', url: "/api/siteplaylists/byId", params: { id, target: ctx.state.sitePlaylistsTarget } };
            return promisedXMLHttpRequest(request).then(data => {
                ctx.commit('setSitePlaylistDetail', JSON.parse(data));
            }).catch(error => { console.log(error) });
        },
        setSitePlaylistsTarget(ctx, target) { ctx.commit('setSitePlaylistsTarget', target) },
        clearSitePlaylistDetail(ctx) { ctx.commit('setSitePlaylistDetail', undefined) },
    }
}
