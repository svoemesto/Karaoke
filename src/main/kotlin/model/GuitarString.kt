...
fun main() {
    ...
    for (i in 0 .. 12) {
        val result = MusicChord.X.getFingerboard(MusicNote.G_SHARP,i)
...
enum class GuitarString(val text: String, val number: Int, val musicNote: MusicNote, val octave: Int, val countFrets: Int = 25) {
    ...
    val fretNextString: Int get() {
        ...
        val interval = MusicNote.getInterval(Pair(currString.musicNote, currString.octave), Pair(prevString.musicNote, prevString.octave))
