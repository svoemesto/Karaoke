  ...
  computed: {
    cards() {
      ...
      return [
        ...
        { label: 'Уник. посетителей', value: s.uniqueVisitors || 0, icon: '👥', color: '#59a14f', sub: 'по anon_id' },
        { label: 'Зарегистр. события', value: s.registeredEvents || 0, icon: '🔑', color: '#f28e2b', sub: `${s.uniqueRegisteredUsers || 0} польз.` },
        ...
        { label: 'Топ-канал', value: s.topChannelCount || 0, icon: '🏆', color: '#ff9da7', sub: s.topChannel || '-' },
      ]
    }
  },
  methods: {
    formatNum(n) { return (n || 0).toLocaleString('ru-RU') }
  }
}
</script>
...
.kpi-label { font-size: 0.78rem; color: #555; }
.kpi-sub {
  font-size: 0.7rem;
  color: #999;
  white-space: nowrap;
