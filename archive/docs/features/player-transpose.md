# Транспонирование аудио в онлайн-плеере (админка + публичный премиум)
...
для отрисовки в сплэше и header.metadata онлайн-плеера (где сдвиг транспонирования
не показывается — только сама тональность):

- Вход `"G minor"` → выход `"Gm"`
- Вход `"A major"` → выход `"A"`
- Вход `"F# minor"` → выход `"F#m"`
...
Реализация — однострочная обёртка вокруг `_parseKey` +
`CHROMATIC[parsed.index] + parsed.suffix` (static, не имеет состояния, идемпотентна).
Согласована с `_transposeLabel` по нормализации бемолей в шарп-эквивалент.

Применяется в 5 точках отрисовки (обе копии `KaraokePlayer.js`, синхронизированы):

- `karaoke-public/src/player/KaraokePlayer.js` — `_renderSplash` (chord desc),
  `_renderHeader` (metadata rows).
- `webvue3/src/player/KaraokePlayer.js` — `_renderSplash` online-режим,
  `_renderSplash` MP4 render-режим (для согласованности экспортируемого mp4 с
  публичной версией), `_renderHeader` (metadata rows).

`_transposeLabel` (для меню «Тональность» и бейджа transpose) остаётся без
изменений — он уже формирует краткую форму, добавляя «(+N)» для сдвига. На
сплэше/header сдвиг не показывается, поэтому используется `_shortKey`. Меню
«Тональность» и бейдж transpose визуально не изменились.

Подробности контракта — в `specs/265-key-short-display/contracts/key-display-contract.md`.

## Публичный плеер (премиум-гейт)

Фаза, изначально отложенная в `spec.md` (FR-017/Assumptions «после
тестирования в админке»), выполнена: транспонирование перенесено из
админ-плеера в публичный (`karaoke-public/src/player/KaraokePlayer.js`) как
**премиум-фича** (PR #142, `107-transpose-public-premium`). Заодно смена
скорости воспроизведения (уже существовавшая для всех) тоже стала
премиум-гейтом (PR #146, `111-public-speed-premium`; текст оверлея исправлен
раздельно на «скорость»/«тональность» в PR #147, `112-premium-overlay-feature`).

- **Гейт**: в `_mode === 'api'` (не blob/inlineData-файл пользователя)
  `setTranspose(n)` при `n !== 0` и `setPlaybackRate(rate)` при `rate !== 1`
  проверяют `this.data?.canExport === true` (та же живая premium-проверка
  бэкендом, что и у экспорта аудио). Не-премиум → `_showPremiumUpsellOverlay('transpose' | 'speed')`
  вместо применения эффекта; возврат к базовым значениям (сдвиг 0 / 1x)
  доступен всем.
- **`_showPremiumUpsellOverlay(feature)`** — canvas-оверлей поверх плеера (по
  образцу `_showDemoEndOverlay`), заголовок/подпись зависят от `feature`,
  кнопка «Оформить подписку →» ведёт на `/premium`.
- **`_restoreTranspose`/`init`** тоже гейтируются: не-премиум не наследует
  ранее сохранённый (localStorage) ненулевой сдвиг/скорость — плеер
  стартует в базовом состоянии, пока пользователь не оформит подписку.
- Отражено в сравнении «FREE vs PREMIUM» на `/premium`
  (`specs/005-free-vs-premium/spec.md` FR-003, строки 8-9) и в
  `docs/strategy/growth-audit.md` (разделы 3.1/4.1).
