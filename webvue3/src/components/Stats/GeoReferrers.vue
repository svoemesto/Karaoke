    ...
    <div class="chart-card">
      ...
      <template v-else-if="referrers.length">
        <table class="table table-sm table-hover ref-table mb-0">
          <thead>
            <tr>
              <th class="text-start">Страница-источник</th>
              <th class="text-end">Переходов</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(r, idx) in referrers" :key="idx">
              <td class="text-start ref-url">
                <a :href="r.name" target="_blank" rel="noopener noreferrer" :title="r.name">{{
                  r.name
                }}</a>
              </td>
...
export default {
  ...
  computed: {
    countryHeight() {
      return Math.max(220, this.countries.length * 26 + 40)
    },
    countrySeries() {
