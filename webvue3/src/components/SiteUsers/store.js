...
export default {
  ...
  getters: {
    getSiteUsersDigest(state) {
      return state.siteUsersDigest
    },
    getSiteUsersDigestIsLoading(state) {
      return state.siteUsersDigestIsLoading
    },
    getSiteUserCurrent(state) {
      return state.siteUserCurrent
    },
    getSiteUserSnapshot(state) {
      return state.siteUserSnapshot
    },
    getSiteUsersTarget(state) {
      return state.siteUsersTarget
    },
    getSiteUserSubscriptions(state) {
      return state.siteUserSubscriptions
    },
    getSiteUserSubscriptionsIsLoading(state) {
      return state.siteUserSubscriptionsIsLoading
    },
    getSiteUsersTableCurrentPage(state) {
      return state.siteUsersTableCurrentPage
    },
  ...
  mutations: {
    ...
    setSiteUsersDigest(state, result) {
      state.siteUsersDigest = result
    },
    setSiteUsersDigestIsLoading(state, isLoading) {
      state.siteUsersDigestIsLoading = isLoading
    },
    setSiteUserCurrentId(state, id) {
      state.siteUserCurrentId = id
    },
    setSiteUserCurrent(state, user) {
      state.siteUserCurrent = Object.assign({}, user)
    },
    setSiteUserSnapshot(state, user) {
      state.siteUserSnapshot = Object.assign({}, user)
    },
    saveSiteUser(state) {
      state.siteUserSnapshot = !state.siteUserCurrent
        ? undefined
        : Object.assign({}, state.siteUserCurrent)
    },
    setSiteUsersTarget(state, target) {
      state.siteUsersTarget = target
    },
    setSiteUserSubscriptions(state, result) {
      state.siteUserSubscriptions = result
    },
    setSiteUserSubscriptionsIsLoading(state, isLoading) {
      state.siteUserSubscriptionsIsLoading = isLoading
    },
    setSiteUsersTableCurrentPage(state, page) {
      state.siteUsersTableCurrentPage = page
    },
  },
  actions: {
    ...
    loadOneSiteUserRecord(ctx, id) {
      let request = {
        method: 'POST',
        url: '/api/siteusers/byId',
        params: { id, target: ctx.state.siteUsersTarget },
      }
      return promisedXMLHttpRequest(request)
        .then((data) => {
          let result = JSON.parse(data)
    ...
    setSiteUserCurrent(ctx, user) {
      ctx.commit('setSiteUserCurrent', user)
    },
    setSiteUserSnapshot(ctx, user) {
      ctx.commit('setSiteUserSnapshot', user)
    },
    setSiteUsersTarget(ctx, target) {
      ctx.commit('setSiteUsersTarget', target)
    },
    saveSiteUser(ctx, diffs) {
      ...
      if (diffs.maxPlaylistItems !== undefined)
        params.maxPlaylistItems = Number(diffs.maxPlaylistItems) || 0
      if (diffs.personalDiscountPercent !== undefined)
        params.personalDiscountPercent = Number(diffs.personalDiscountPercent) || 0
      ...
      if (diffs.sponsrPremiumUntil !== undefined)
        params.sponsrPremiumUntil = diffs.sponsrPremiumUntil || ''
      if (diffs.sitePremiumUntil !== undefined)
        params.sitePremiumUntil = diffs.sitePremiumUntil || ''
      if (diffs.welcomeMessageSent !== undefined)
        params.welcomeMessageSent = diffs.welcomeMessageSent
    ...
    banSiteUserCurrent(ctx, reason) {
      let request = {
        method: 'POST',
        url: '/api/siteusers/ban',
        params: {
          id: ctx.state.siteUserCurrentId,
          target: ctx.state.siteUsersTarget,
          reason: reason || '',
        },
      }
      return promisedXMLHttpRequest(request).then(() =>
        ctx.dispatch('loadOneSiteUserRecord', ctx.state.siteUserCurrentId),
      )
    },
    unbanSiteUserCurrent(ctx) {
      let request = {
        method: 'POST',
        url: '/api/siteusers/unban',
        params: { id: ctx.state.siteUserCurrentId, target: ctx.state.siteUsersTarget },
      }
      return promisedXMLHttpRequest(request).then(() =>
        ctx.dispatch('loadOneSiteUserRecord', ctx.state.siteUserCurrentId),
      )
    ...
    loadSiteUserSubscriptions(ctx, id) {
      const request = {
        method: 'POST',
        url: '/api/siteusers/subscriptions',
        params: { id, target: ctx.state.siteUsersTarget },
      }
