...
- Правила взаимоисключающие: каждая песня при каждом пересчёте получает ровно одно из пяти
  состояний (FR-007).
- `IN_WORK` имеет пустой цвет на уровне enum (`SongState.IN_WORK.color == ""`) — это «чистое»
  состояние для DTO/логики. **UI-презентация** для песен со `idStatus < 6L` берёт цвет из
  палитры по стадиям жизненного цикла через `Song.colorByIdStatus(status)` (NONE / текст
  найден / текст проверен / проект создан / проект проверен / рендер готов) — это
  восстановленная старая логика (`Song.loadListFromDb` ставит `colorByIdStatus(idStatus)`,
  если песня в работе). Готовые песни используют цвет из `SongState.color`.
...
- **Цвет строки в работе зависит от `idStatus`**: песня со `idStatus < 6L` (т.е.
  `Song.state == IN_WORK`) получает цвет **не из enum-IN_WORK** (он `""`), а из
  `Song.colorByIdStatus(idStatus)`. Палитра: `#FFFFFF` (NONE) → `#DDA0DD` (текст найден) →
  `#EE82EE` (текст проверен) → `#98FB98` (проект создан) → `#00FF7F` (проект проверен) →
  `#D2691E` (рендер готов). Без подстановки цвета готового состояния. Если в UI строка
