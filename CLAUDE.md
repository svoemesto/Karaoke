...
- **Онлайн-редактор разметки (`/account/editor`) с модерацией.** Упрощённый публичный аналог `SubsEdit.vue`:
  ...
  рабочий цикл (назначить→работа→апрув) часто идёт ЦЕЛИКОМ на PROD — `assign`/`approve`
  (`SongEditorController`) target-aware (local|remote); единственное, что апрув всегда пишет в LOCAL —
  само изменение песни в `tbl_settings` (karaoke-app рендерит только с локального диска). Композитный
  статус — `SongAssignmentStatus.resolve(...)` с timestamp'ами. Доступ к разделу — отдельная роль
  `is_editor` на `SiteUser` (не факт наличия assignment'а), включается админом в webvue3 по образцу
  премиума; гейтинг на фронте (кнопка/редирект) и **обязательно** на бэкенде (`PublicSongEditorController`,
  иначе снятие роли не отзывает доступ к уже выданным заданиям). Детали →
...
- **Монетизация (подписки).** Три способа премиум-доступа — везде термин **«подписка»**, не «покупка»:
  ...
  блокирует кнопку «Оплатить» на фронте, пока ключи ЮKassa не заданы. Оферта — `/oferta`. Детали → memory
  `project_monetization_subscriptions`.
