...
4. **Движок поиска обложки альбома** — отдельный выбор (enum
   ...
   **Один HTTP-клиент на весь модуль обложек** (Pass 459): и поиск
   (SearXNG/fourget), и загрузка картинок-кандидатов идут через общий
   `albumCoverHttpClient`. Раньше `downloadImageBytes` создавал свой
   `HttpClient` на каждый вызов, а вызывается он из прокси-эндпоинта
   `/api/song/albumcoverproxy` на **каждую** картинку галереи кандидатов — то
   есть до 24 клиентов со своими пулами соединений и рабочими потоками на один
   поиск обложки.
...
6. **Скрейпинг** — для каждого результата парсим HTML:
   ...
   - **Текст для LLM сохраняет переводы строк** (`LyricsFinderService.htmlToStructuredText`,
     Pass 457): маркер вставляется в местах `<br>` и на границах блочных элементов,
     затем `Element.text()` нормализует пробелы, и уже после этого маркер
     превращается в настоящий `\n`. До Pass 457 брался `doc.body().text()`, а он
     схлопывает страницу в **одну строку** — при том что системный промпт
     `ScraperAgent` требует «Preserve … line breaks» и прямо говорит, что «lyrics …
     look like short lines separated by line breaks». Из 26497 заполненных
     `source_text` в рабочей БД 4926 (18.6%) не имеют переводов строк вообще.
   - Обрезка длинного текста — `truncateForLlm`, лимит `MAX_PAGE_TEXT_CHARS` (8000)
     и **по границе строки**, а не по символу: иначе последняя строка уходит в
     модель обрубленной посередине слова.
7. **LLM-анализ** — `ScraperAgent.kt` через `LmStudioService.kt` (тонкий клиент над
   ...
   - **LangChain4j убран целиком** (Pass 462). Зависимость
     `langchain4j-spring-boot-starter` держалась ровно ради аннотации `@Tool` на
     `SearchTool.searchUrls`, но потребителя у аннотации не было: AI-сервисов
     (`AiServices`) в проекте нет, а все LLM-вызовы идут напрямую через
     `LmStudioService`. То есть аннотация была инертной, а стартер — мёртвым весом
     (три jar-а ~457 КБ плюс автоконфигурация Spring Boot в сканировании). Если
     когда-нибудь появится настоящий tool-calling — зависимость придётся вернуть
     осознанно, вместе с кодом, который её использует.
8. **Яндекс.Музыка** — отдельный путь (`searchLastAlbumYm3`/
   ...
   - **Один запуск браузера, а не два** (Pass 458). Раньше и здесь, и в
     `fetchYandexArtistAlbumsHtml` (`AlbumCoverFinder.kt`) подряд шли два
     `Playwright.create()`: первый открывал `music.yandex.ru/`, закрывался, и
     результат не использовался; второй делал работу. Теперь запуск один, а
     переход на главную сохранён — он мог быть задуман как «прогрев» сессии в
     persistent-профиле, и намерение нигде не задокументировано. Поэтому
     изменение безопасно при любой трактовке, а полный запуск Chromium (секунды
     и сотни мегабайт на каждый вызов) больше не тратится впустую.
9. **VPN-детект** — `isVpnActive()`: страна текущего внешнего IP (через
   ...
   кэш — общий `PollingCache`, Pass 456). Причина кэша — не частота вызовов (она
   невелика: `KaraokeProcessWorker` ограничен `requestNewSongTimeoutMs` = 10 мин,
   `AutoOneClickSyncScheduler` — раз в минуту), а квота резервного сервиса:
   `ipapi.co` даёт ~1000 запросов/сутки против 1440 при вызове раз в минуту, и его
   исчерпание ломает детект ВПН (fail-open). На 2026-09-26 резерв отвечал HTTP 429 —
...
- **`lyricsSearchTimeoutSeconds` — это НЕ таймаут запроса** (Pass 460). Настройка
  задаёт **паузу между последовательными поисковыми запросами** в пакетном поиске
  (`Thread.sleep(timeout * 1000L)` между итерациями), а не предел длительности
  одного запроса. Имя и ключ вводили в заблуждение, но ключ переименовать нельзя:
  в живом `Karaoke.properties` он уже выставлен оператором (20), и смена ключа
  молча вернула бы дефолт 10. Фактические таймауты HTTP задаются отдельно — на
  клиенте (`connectTimeout`) и на запросе (`.timeout(...)`). Чтение с полом 10
  живёт в одном месте — `Karaoke.lyricsSearchTimeoutSeconds`; до Pass 460 то же
  выражение было скопировано ещё в 4 местах контроллеров, поэтому правка в одном
  месте не доезжала до остальных. В UI поле подписано «Пауза между запросами (сек)».
...
6. **Tracking-маркеры в query**:
   ...
   отбрасываются.
   **Маркер обязан начинаться на границе параметра** — сразу после `?` или `&`
   (`matchesUselessUrlPattern`, Pass 461). Раньше проверка была голым `contains`
   по всему URL, поэтому `ref=` срабатывал и на честном `?pref=...` — живая ссылка
   на текст песни отбрасывалась как мусор. Заодно перестала быть ложной
   отрицательностью запись маркера: в дефолтном списке часть маркеров записана с
   ведущим `?` (`?utm_source=`), часть без (`ref=`, `fbclid=`). Теперь `?`/`&`
   в паттерне необязателен, а граница параметра проверяется в URL, поэтому
   `...?id=1&utm_source=vk` тоже отбрасывается (раньше — нет). Паттерны без `=`
   (служебные path, расширения файлов) проверяются подстрокой, как и раньше.
