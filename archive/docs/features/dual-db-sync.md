...
## Синхронизация временных ссылок

`tbl_song_share_links` зарегистрирована как `sharelinks` с направлением one-click
`SERVER_TO_LOCAL` и отдельными восемью флагами операций. Синхронизируются метаданные ссылки,
срок действия, состояние отзыва и непрозрачные hash-поля; hash-секреты не входят в label,
summary или пользовательские логи. `tbl_song_share_sessions` намеренно не входит в
`SyncRegistry`: это краткоживущие lease/session и аудит использования, перенос которых может
восстановить устаревшее runtime-состояние.
...
- **Найдена и устранена настоящая причина (2026-07-30, specs/098-fix-approve-blocking-checkandannounce)**:
  тайминги из предыдущего пункта показали `checkAndAnnounce`, синхронно выполняющийся **58 секунд**
  внутри HTTP-запроса `approve()` — на проде накопилось 4126 песен с `id_status >= 6`, ни разу не
  отмеченных анонсированными (`tbl_song_news_announced` содержала 11123 из 15249 «готовых»), и
  `forEachNewlyReadyCandidate` каждый раз заново загружает и фильтрует их все (166 чанков по 25).
  ...
  daemon-потоке (`AtomicBoolean`-гвард от параллельных запусков) — HTTP-ответ администратору больше
...
- **Устранено ложное срабатывание «Доступна» после отложенной синхронизации (2026-08-05,
  specs/152-fix-false-collection-news)**: описанная выше идемпотентность «доступна» («переход
  флага false→true по сравнению с текущим значением на этой БД») оказалась ненадёжной — если
  целевая БД впервые узнаёт уже давно истинное значение `newsAvailableAnnounced` (например, из-за
  того, что одноразовый backfill выше не покрыл конкретную запись, или предыдущий push этого поля
  не прошёл), локальный переход `false → true` фиксируется, хотя реального нового события не было.
  На проде это проявилось как новость «в коллекции», вышедшая ПОСЛЕ уже опубликованной новости «в
  эфире» по тем же 3 песням — притом что «в эфире» логически подразумевает, что песня уже давно
  доступна. Исправлено в `detectAndAnnounceAvailability`: `wasAvailableBefore` теперь только
  дешёвый fast-path, а не гейт корректности; перед созданием новости `category="premium"`
  добавлены два содержательных гейта через `News.existsAnnouncement` (тот же паттерн, что уже
