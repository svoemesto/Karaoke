...
object Mp3Trimmer {
    ...
    private fun parseFrameHeader(bytes: ByteArray, pos: Int): FrameInfo? {
        ...
        val sampleRateTable = when (versionBits) { 3 -> SAMPLERATES_V1; 2 -> SAMPLERATES_V2; else -> SAMPLERATES_V25 }
    ...
    private fun isXingFrame(bytes: ByteArray, pos: Int, frameLength: Int, isMpeg1: Boolean, b3: Int): Boolean {
        val channelMode = (b3 shr 6) and 0x3
        val isMono = channelMode == 3
        val sideInfoSize = if (isMpeg1) { if (isMono) 17 else 32 } else { if (isMono) 9 else 17 }
