...
MonitorCheck` (сейчас 5: горизонт запланированных постов в Telegram, доступность прод-сервера,
остановленная очередь рендера, выключенный Telegram-поллинг, непрочитанные сообщения в чате от
пользователей; добавление новой проверки — одна строка в реестре). `MonitoringService`
...
- **«Чат с автором проекта».** Личная переписка премиум-пользователей сайта с автором — один
  ...
  Пользователь пишет из `/account/chat` (karaoke-public); доступ на чтение истории — всем
  авторизованным, на отправку — только активному премиуму (истёкший видит read-only + плашку с
  апселлом). Автор — из раздела «Чат» в webvue3: двухпанельный вид (список тредов слева + поиск
