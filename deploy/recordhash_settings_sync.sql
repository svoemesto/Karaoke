ALTER TABLE tbl_settings_sync
ADD COLUMN IF NOT EXISTS recordhash VARCHAR(32);

CREATE INDEX IF NOT EXISTS idx_tbl_settings_sync_recordhash ON tbl_settings_sync(recordhash);

CREATE OR REPLACE FUNCTION update_tbl_settings_sync_recordhash()
RETURNS TRIGGER AS
$$
BEGIN
    NEW.recordhash = md5(
...
CREATE OR REPLACE TRIGGER update_recordhash_settings_synctrigger
    BEFORE UPDATE OR INSERT
    ON tbl_settings_sync
    FOR EACH ROW
EXECUTE FUNCTION update_tbl_settings_sync_recordhash();

UPDATE tbl_settings_sync
SET recordhash = md5(
        COALESCE(id::TEXT, '') ||
        COALESCE(song_name, '') ||
