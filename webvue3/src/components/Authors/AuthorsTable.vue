...
export default {
  ...
  computed: {
    ...
    isSavingRowsPerPage() {
      return this.$store.getters.isSavingRowsPerPage('authors')
    },
    countRows() {
  ...
  async mounted() {
    ...
    // ДО любых действий с perPage, чтобы данные загружались с правильным значением.
    await this.$store.dispatch('loadTableSettings')
    this.perPage = this.$store.getters.getRowsPerPage('authors')
  ...
  methods: {
    ...
    async onPerPageChange(e) {
      ...
      if (isNaN(parsed) || parsed < 1 || parsed > 1000) {
        ...
        console.warn('[AuthorsTable.onPerPageChange] invalid value', rawValue)
        return
      }
      if (parsed === this.perPage) return
      this.currentPage = 1 // ADR-0004: page reset
      if (this.$store.getters.getAuthorsTableCurrentPage !== undefined) {
        this.$store.commit('setAuthorsTableCurrentPage', 1)
      }
      ...
      if (ok) {
        this.perPage = this.$store.getters.getRowsPerPage('authors')
        await this.$store.dispatch('loadAuthorsDigests', {
          page: this.currentPage,
          perPage: this.perPage,
          ...this.$store.getters.getAuthorsFilter,
        })
      }
    },
    editPicture(id) {
...
.authors-bv-table-header .pagination,
.authors-bv-table-header ul.pagination {
