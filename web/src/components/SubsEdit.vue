              ...
              <div class="grid-item-controls">
                <div class="group-edit-play-speed-buttons">
                  <div class="group-edit-speed-buttons">
                    <label for="group-edit-speed-buttons" class="label-for-group-edit-speed-buttons">Edit:</label>
                  ...
                  <div class="group-play-speed-buttons">
                    <label for="group-play-speed-buttons" class="label-for-group-play-speed-buttons">Play:</label>
                ...
                <div class="group-actions-buttons">
                  ...
                  <button class="group-button" type="button" @click="save">
                    <img alt="save" class="icon-40" title="Save" src="../assets/svg/icon_save.svg">
                  </button>
                </div>
...
export default {
  ...
  watch: {
    ...
    pressedBL: { handler () { if (!this.isEditMode) return; if (this.pressedBL) { this.goToPreviousMarker(); } } },
    pressedBR: { handler () { if (!this.isEditMode) return; if (this.pressedBR) { this.goToNextMarker(); } } },
    isEditMode: {
      handler () {
        if (this.isEditMode) {
  ...
  computed: {
      ...
      for (let i = 0; i < markers.length; i++) {
        ...
        switch (marker.markertype) {
          ...
          default: {
            ...
            let txt = marker.label.replaceAll('_', ' ');
            if (wasBr) {
    ...
    getTail() {
      ...
      for (let i = this.currentSyllablesIndex; i >= 0; i--) {
        textBegin = this.sourceSyllables[i] + textBegin;
  ...
  methods: {
    ...
    updateMarkersBySyllables() {
      ...
      for (let i = 0; i < this.sourceMarkers.length; i++) {
        ...
        if (marker.markertype === 'syllables') {
          if (marker.label !== this.sourceSyllables[index] || marker.color !== color) {
    ...
    async loadSong() {
      ...
      await this.ws.load('/apis/song/' + this.song.id + '/file' + this.sound).then(() => {
