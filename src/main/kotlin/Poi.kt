    ...
    companion object {
        ...
        fun getSettingFields(author: String, songName: String, spreadsheetDocument: SpreadsheetDocument): MutableMap<SettingField, String>? {
            ...
            if (row == null) {
                ...
                val time = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_TIME)).stringValue.trim()
                ...
                val year = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YEAR)).stringValue.trim()
                val album = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_ALBUM)).stringValue.trim()
                val track = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_TRACK)).stringValue.trim()
                val tone = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_TONE)).stringValue.trim()
                val bpm = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_BPM)).stringValue.trim()
                val format = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_FORMAT)).stringValue.trim()
        ...
        fun getSongVKDescription(song: Song, fileName: String, spreadsheetDocument: SpreadsheetDocument?): Pair<String, String>? {
            ...
            if (spreadsheetDocument != null) {
                ...
                if (row == null) {
                    ...
                    date = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_DATE)).stringValue.trim()
                    time = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_TIME)).stringValue.trim().replace(":",".")
                    boostyNormal = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_BOOSTY)).stringValue.trim()
                    lyricsNormal = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_LIRIC)).stringValue.trim()
                    lyricsDelay = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_LIRIC_BT)).stringValue.trim()
                    karaokeNormal = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_KARAOKE)).stringValue.trim()
                    karaokeDelay = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_KARAOKE_BT)).stringValue.trim()
                    chordsNormal = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_CHORDS)).stringValue.trim()
                    chordsDelay = row.getCellByIndex(Ods.getColumnIndex(firstRow!!, ODS_COLUMN_YOUTUBE_CHORDS_BT)).stringValue.trim()
                }
            } else {
