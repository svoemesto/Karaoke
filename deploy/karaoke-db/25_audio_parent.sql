-- "Аудио-родитель" песни: id наиболее похожей по аудио-анализу песни (WaveformCompare),
-- % схожести и дельта сдвига маркеров (мс) — независимо от кураторского root_id.
-- audio_compare_history — JSON-история уже выполненных сравнений (id кандидата, %, дельта,
-- ok, время), чтобы не сравнивать повторно одни и те же пары при последующих запусках поиска.
--
...
--    (даже без реальных изменений). Оба триггера пересоздаются тут (см. karaoke-db/24_song_type.sql).

ALTER TABLE public.tbl_settings
    ADD COLUMN IF NOT EXISTS audio_parent_id integer NOT NULL DEFAULT 0;
ALTER TABLE public.tbl_settings
    ADD COLUMN IF NOT EXISTS audio_similarity_percent integer NOT NULL DEFAULT 0;
ALTER TABLE public.tbl_settings
    ADD COLUMN IF NOT EXISTS audio_delta_ms bigint NOT NULL DEFAULT 0;
ALTER TABLE public.tbl_settings
    ADD COLUMN IF NOT EXISTS audio_compare_history text NOT NULL DEFAULT '[]';
...
CREATE INDEX IF NOT EXISTS idx_tbl_settings_audio_parent_id ON public.tbl_settings(audio_parent_id);
