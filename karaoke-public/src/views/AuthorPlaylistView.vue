  ...
  <div class="km-page">
    <header class="km-header">
      <div class="km-header-inner">
        <RouterLink :to="{ path: '/zakroma', query: author ? { author } : {} }" class="km-back">← Закрома</RouterLink>
        <a href="/"><img src="/KARAOKE_LOGO.png" class="km-logo" alt="Karaoke logo" /></a>
      </div>
    </header>

    <LoginRequired v-if="!isLoggedIn" text="Плейлисты авторов доступны только зарегистрированным пользователям. Войдите или зарегистрируйтесь — это бесплатно." />
    ...
    <div v-else class="km-content">
      ...
      <div class="km-player-box" :class="{ 'km-player-wide': player.playerWide.value }">
        ...
        <div v-else class="km-player-placeholder">
          <button class="km-big-play" :disabled="!hasPlayable" @click="onStart">▶ Запустить плейлист</button>
          <p v-if="!hasPlayable" class="km-player-hint">Нет доступных вам для воспроизведения песен этого автора.</p>
        </div>
      </div>

      <!-- Управление -->
      <div class="km-controls">
        <button class="km-ctrl-btn" title="Предыдущая" :disabled="!player.started.value" @click="player.prev">⏮</button>
        <button class="km-ctrl-btn km-ctrl-main" :title="player.isPlaying.value ? 'Пауза' : 'Играть'" @click="onToggle">
          {{ player.isPlaying.value ? '⏸' : '▶' }}
        </button>
        <button class="km-ctrl-btn" title="Следующая" :disabled="!player.started.value" @click="player.next">⏭</button>
        <div class="km-ctrl-sep" />
        <button class="km-ctrl-toggle" :class="{ active: modes.continuous }" title="Непрерывное воспроизведение" @click="toggleContinuous">⇥ Непрерывно</button>
        <button class="km-ctrl-toggle" :class="{ active: modes.repeatMode !== 'none' }" :title="repeatTitle" @click="cycleRepeat">{{ repeatLabel }}</button>
        <button class="km-ctrl-toggle" :class="{ active: modes.shuffle }" title="Случайный порядок" @click="toggleShuffle">🔀 Случайно</button>
      </div>
...
export default {
  ...
  setup() {
    ...
    const repeatTitle = computed(() =>
      modes.repeatMode === 'one' ? 'Повтор одной песни'
        : modes.repeatMode === 'all' ? 'Повтор всего плейлиста' : 'Повтор выключен')
    ...
    async function loadSongs() {
      ...
      for (const block of (data || [])) {
        ...
        for (const alb of albums) {
          const setts = [...(alb.albumSettings || [])].sort((a, b) => (a.track || 0) - (b.track || 0))
          for (const s of setts) flat.push({ songId: s.id, songName: s.songName, album: alb.albumName, year: alb.year, track: s.track })
        }
      }
      songs.value = flat
      loading.value = false
    ...
    function onStart() { player.start(playableIds(), modes) }
    function onToggle() { if (!player.started.value) onStart(); else player.toggle() }
    function toggleContinuous() { modes.continuous = !modes.continuous; player.pushModes(modes) }
    function cycleRepeat() {
      modes.repeatMode = modes.repeatMode === 'none' ? 'all' : modes.repeatMode === 'all' ? 'one' : 'none'
      player.pushModes(modes)
    }
    function toggleShuffle() { modes.shuffle = !modes.shuffle; player.pushQueue(playableIds()) }
...
.km-name-row { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem; }
...
.km-note { font-size: 0.8rem; color: var(--km-text2); margin: 0 0 1rem; }
...
.km-player-box.km-player-wide { position: fixed; inset: 0; z-index: 2000; margin: 0; border-radius: 0; aspect-ratio: unset; width: 100vw; height: 100vh; }
